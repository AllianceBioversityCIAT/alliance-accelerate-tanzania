...
Two OSM tile surfaces now exist: the public discovery map (`LeafletMap`) and the coordinate picker (`CoordinatePickerMap`, mounted through `CoordinatePicker`). Both import their geography and tile source from **`components/map/map-constants.ts`** — `TANZANIA_CENTER`, `TANZANIA_BOUNDS`, `OSM_TILE_URL`, `OSM_ATTRIBUTION`. **Never re-type the attribution string**: two public surfaces with divergent attribution is an OpenStreetMap licence-compliance defect, not untidiness. The exports are `readonly` tuples **on purpose**, so the module stays importable by code that must not pull `leaflet` in; Leaflet's `LatLngTuple` is `[number, number, number?]` and carries no `readonly`, so a consumer spreads at the call site (`[...TANZANIA_CENTER]`) — expect `TS2345` on a direct pass and spread, don't loosen the shared contract.
