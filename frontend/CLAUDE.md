...
**`components/contact/` partitions errors solely on `Array.isArray(err.details)`** — it never reads `err.message` or `err.status` (FR-5). Adding a branch on either is a behaviour change, not a refinement.

**Never infer rendered geometry from class inspection (`contact/contact-channels`).** Any *does it fit / overflow / crowd* question is answered by measuring `scrollWidth` vs `clientWidth` in a real browser, never by estimating glyph widths from Tailwind classes. Note that a `max-w-*` container caps usable width **permanently**, so "show it at a wider breakpoint" is frequently impossible rather than merely undesirable.

## Map surfaces (`components/map/`) — two silent-failure traps

Two OSM tile surfaces now exist: the public discovery map (`LeafletMap`) and the coordinate picker (`CoordinatePickerMap`, mounted through `CoordinatePicker`). Both draw their tile source from **`components/map/map-constants.ts`**, which exports `TANZANIA_CENTER`, `TANZANIA_BOUNDS`, `OSM_TILE_URL`, `OSM_ATTRIBUTION`. ⚠️ **They do not import the same set:** `LeafletMap` uses all four; `CoordinatePickerMap` imports **three** — it frames its empty view with `fitBounds(TANZANIA_BOUNDS)` and never references `TANZANIA_CENTER`. Editing `TANZANIA_CENTER` moves `/map` only. **Never re-type the attribution string**: two public surfaces with divergent attribution is an OpenStreetMap licence-compliance defect, not untidiness. Two independent properties, often conflated: the exports are typed as **plain tuples rather than Leaflet's `LatLngExpression`/`LatLngBoundsExpression`**, which is what keeps `leaflet` out of importers — and they are **`readonly`**, which protects shared values from consumer mutation and is what *causes* the type error; Leaflet's `LatLngTuple` is `[number, number, number?]` and carries no `readonly`, so a consumer spreads at the call site. **One spread is not always enough:** `[...TANZANIA_CENTER]` suffices for the point, but `TANZANIA_BOUNDS` is a pair of pairs whose *inner* tuples stay `readonly` — both call sites use `TANZANIA_BOUNDS.map((corner) => [...corner])`. Expect `TS2345` on a direct pass; spread, don't loosen the shared contract.

⚠️ **Mount a Leaflet map only into a laid-out container.** `fitBounds` derives its zoom by dividing by `getSize()`, so a `0×0` container resolves to **zoom 0 — the whole world — and stays there for the map's lifetime**, because `invalidateSize` preserves centre and zoom and never re-fits. Render the map **conditionally** (`{open && <Map/>}`), never render-and-hide behind `display:none`, `hidden`, a breakpoint class, a collapsed section, or a zero-height wrapper. Nothing errors when this happens.

⚠️ **The picker's two-way binding compares coordinates exactly (`===`), so nothing may normalize a `LatLng`.** `lib/geo/coordinates.ts`'s `isSamePoint` guard settles in one pass only because the marker is set from the parsed value verbatim and read back via `getLatLng()`. Enabling `worldCopyJump`, calling `.wrap()`, or re-deriving position from pixel coordinates shifts the double and makes the guard report "different" forever — the map fights the fields. Wrap a longitude only as a **number on the write path**, before formatting (`L.Util.wrapNum(lng, [-180, 180], true)`), never on a `LatLng` the guard will read.

**`lib/geo/coordinates.ts` is the coordinate seam** — `COORDINATE_PRECISION` (5), the range constants, `formatCoordinate`, `parseCoordinatePair`, `isSamePoint`. Pure over strings and numbers; it imports nothing from `leaflet`, `react`, or any component, and that is what makes the picker's rules provable in jsdom. Keep it that way: a test that needs `jest.mock('leaflet')` to run has broken the seam, not satisfied it. Everything Leaflet actually *does* — marker creation, drag, click wiring, map fit — is **unevaluable in jsdom** (no layout engine) and must be verified in a real browser, never asserted by a test that cannot fail.
