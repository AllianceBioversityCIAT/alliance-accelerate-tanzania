...
## CodeGraph
**Per-checkout, and not initialized by default — check before relying on it.** Only `.codegraph/config.json` is committed; the generated database is gitignored, so a fresh clone has **no graph** and every `codegraph_*` call fails with *"CodeGraph not initialized"* until someone runs `codegraph init` in that working copy. Verified absent in this checkout on 2026-08-04. *(This section previously asserted "Re-indexed 2026-08-03 — 290 files, 2,688 nodes, 6,881 edges", which described a machine where the index had been built and sent every agent to tools that cannot run here. Corrected at `/akili-archive` by the factual-claims sweep. Do not restore per-checkout counts to a shared guide — they are not a property of the repository.)*
