...
- **Before starting or resuming a task, check whether another local branch already touches the files it will change** (`git log --oneline --all -20 -- <paths>` or equivalent). This rule is otherwise advisory-only and unenforced — a multi-week, 13-commit divergence across two branches editing the same forms went undetected until a user compared two screenshots (KZ-010).
  - **Corollary — a shared counter is not free just because you allocate it from `main`.** ADR numbers, and any other monotonic id in a shared document, must be checked against **unmerged branches** before allocation: `git log --oneline --all -20 -- docs/trd/trd.md`, then read the candidate number in the newest branch commit that touched it. The apply-phase gate ("allocate at apply time on the default branch, never from a spec branch") prevents a *branch* from allocating a number the trunk will later reuse — it does **not** make the number free, it only decides **who pays the renumbering**, and the answer is always the branch. Following the gate exactly and skipping this check still collides: ADR-011 was allocated on `admin/registration-review-queue` on 2026-09-01 23:46 and again on `main` on 2026-09-02 09:02, costing the branch a 14-citation forward sweep. Neither allocation was wrong; the missing step was this one.
