 ...
 * resolution of its own. One departure from that shape, scope-driven:
 *
 * **`to` is a parameter, not resolved here.** `buildReceiptMessage` is
 ...
 *
 * *(Until enhancement/email-notification-microservice T-11, a second
 * departure existed: `replyTo` was set via `composeReplyTo`, so an
 * administrator could reply directly to the visitor. That field is now
 * removed end-to-end — `reply-to.util.ts` deleted, `MailMessage` no longer
 * carries the field — because the notification-microservice DTO has no
 * reply-to field to carry it into (design.md DD-7). CC-ing the visitor was
 * rejected outright as the replacement: it would send them the admin-facing
 * message, disclosing the internal note format and the resolved
 * administrator recipients.)*
 *
 * **Header/body split (FR-4's own boundary).** No header this template
 * produces carries requester-supplied text: `to` is the resolved
 * administrator list, and `CONTACT_SUBJECT` (the `Subject:` field every
 * transport sends) is a fixed, server-generated constant. Every
 * requester-supplied value — name, organization, category, the visitor's
 * own "subject" field, message, and the visitor's address — is rendered as
 * BODY DATA instead. `CONTACT_SUBJECT` must not be confused with the DTO's
 * own `subject` property, which is a plain body field like `message`.
 *
 ...
 * `CONTACT_PROVENANCE_LINE` is the fixed opening statement that this is a
 * public, unverified submission, so no administrator reading it mistakes
 * the sender's stated identity — name OR the restated email address that
 * follows it — for a verified one. `CONTACT_PROVENANCE_LINE` itself carries
 * NO interpolated data (so its exact text is a stable, assertable
 * constant); the address is rendered as an ordinary labelled body field
 * immediately below it, per FR-4's "renders the requester's address as
 * body data".
 *
 ...
 * `message` reaches only the rendered body text — this template's only two
 * headers are `to` (the resolved administrator list) and `subject`
 * (`CONTACT_SUBJECT`, a fixed constant); under the operating
 * `MicroserviceMailTransport` the value lands at
 * `envelope.data.data.emailBody.message.text` (`microservice-mail.transport.ts`'s
 * `buildMicroserviceEnvelope`), never in a header field, so a newline
 * inside it is just a newline and cannot inject or spoof anything.
 * *(This file previously stripped `message` too, "exactly like
...
/**
 * The fixed `Subject:` field the transport sends for every contact-form
 * message. Contains no requester-supplied text (FR-4) — the visitor's own
 * "subject" field is rendered as body data instead, see `renderBody` below.
 */
