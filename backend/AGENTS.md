...
5. **Migrations:** additive-only by default. Rehearse on whatever MySQL 8 `backend/.env` `DATABASE_URL` points at — in practice usually the **shared dev RDS**, since most checkouts have no local MySQL (a local docker MySQL is preferred when running, but is not a precondition). Know that `migrate dev` reads the URL from `.env` and **provisions a shadow database** on its target: fine on dev RDS, never on PROD. A reset or drift prompt is **abort-and-report**; `migrate reset`/`db push` are forbidden against RDS. Governed/PROD apply uses `prisma migrate deploy` + in-process secret composition (pattern in `../infra/scripts/migrate-seed.sh`; that script also seeds — never run it whole on live data). *(Amended 2026-08-05 — see `backend/CLAUDE.md` § Data & migrations for why.)*
