...
**Decided 2026-09-23, before T-4, by reading the microservice rather than assuming.** §3 step 5 said "publish" without saying how, and the obvious worry — that a Lambda would need the backend's 785-line transport — turns out not to follow.
...
**The one datum this spec has, per `mail/mail-timing.ts`'s discipline that a timing budget belongs in one place — this is that place, *the canonical home*. `tasks.md` §5's NFR-6 row and `execution.md`'s corrected T-7 entry both cite the figure and both point here; neither is authoritative and neither is updated if it changes. *(This clause previously claimed the number appeared nowhere else in the spec. It appears in both of those files and three times in `validation-report.md` — a self-referential uniqueness claim published without the grep that would have checked it, the sixth of that exact class here. Deferral is the true and stronger discipline; uniqueness was neither. **Corrected again 2026-09-25, this time by a repo-wide grep rather than a spec-folder-only one:** the raw figure also stood at `docs/trd/trd.md`'s C4 arrow correction, stated there against the exact denominator this clause disqualifies — *"against a 15 s timeout"* — because the sweep that found the other two sites never left the spec folder. Fixed at that file to point here instead.)*:** T-7's CloudWatch measurement against the real deploy, `Duration: 1953.61 ms | Init: 598.71 ms`, **≈2.55 s of cold-path latency this spec introduces against Cognito's trigger ceiling** — a ceiling of unknown size, not against this function's own 15 s.
...
⚠️ **Scope note (validation-report.md W-14), recorded here because this is the decision it qualifies.** `guard-account.no-account-id-literal-in-infra` scans `infra/` only, so it is silent about — and cannot be evidence against — the same literal (`569113802249`) surviving in prose: it remains in this document's own B-1 paragraph above, in `execution.md`'s CLI-output evidence, and in the committed `pool-before.json`/`pool-after.json` artefacts (both legitimately, as verbatim live-pool captures). The green gate certifies "not versioned under `infra/`," not "not versioned anywhere" — a narrower claim than a reader may assume. Left as an accepted, named state rather than remediated here, since remediating it is a separate, larger decision (redact docs vs. widen the guard's scope) that this task was not asked to make. `validation-report.md` itself also carries the literal and needs the same note; that document is the Leader's, not edited here.
...
**Verified, not assumed** — but the verification proved the wrong half of the claim. **⚠️ CORRECTED post-deploy, validation-report.md B-4:** `users.service.ts::update()` sends `AdminUpdateUserAttributesCommand` with a new `email`, and the pool carries `AutoVerifiedAttributes: ["email"]` — that reachability was, and remains, correctly established **at the commit measured** (`acb25ad`): an admin editing a user's address **did** cause this trigger to fire. What was never independently checked is *which* Cognito `triggerSource` name that mechanism uses. **It is measured live in DEV to be `CustomEmailSender_UpdateUserAttribute`, not `CustomEmailSender_VerifyUserAttribute`.** Round-1 C-6 established that raising on the admin-edit source would break that shipped admin feature — that conclusion still holds, it was just attached to the wrong row.

⚠️ **Superseded 79 minutes later, same day, by `2960d74` — kept because the break and the fix it prompted were both real when measured.** `2960d74` is an unrelated, owner-requested change: `update()` now sets `{ Name: 'email_verified', Value: 'true' }` in the **same** `AdminUpdateUserAttributesCommand` as the new `email`, so an admin-edited address is already verified and Cognito has nothing left to verify. Cognito therefore now emits **no** `CustomEmailSender_*` trigger for this path at all. The row below, and the addendum after the table, describe the mechanism as it stood between this measurement and `2960d74` — real for that window, and the reason the handler exists — not as a description of what `update()` does today. Handling `UpdateUserAttribute` remains correct regardless (§10's C-6/DD-2 reasoning does not depend on the path being live): a correct handler for a currently-unreached source costs nothing.

| `triggerSource` | Decision |
|---|---|
| `CustomEmailSender_ForgotPassword` | **Handle** — the purpose of this spec. |
| `CustomEmailSender_UpdateUserAttribute` | ⚠️ **Handle — corrected post-deploy, see the addendum below.** This, not `VerifyUserAttribute`, is the source `users.service.ts::update()`'s `AdminUpdateUserAttributesCommand` measurably emitted against this auto-verified pool **as of the commit measured**. Same shape, same message builder. Raising here would break the admin email-edit path — the risk C-6 named, on the right mechanism, under the wrong name. ⚠️ **Stale within the day, `2960d74`:** `update()` now sets `email_verified: 'true'` in the same command, so Cognito has nothing to verify and this path fires no trigger any more — kept `Handle`d (correct and free), but now, like `VerifyUserAttribute` below, **not reachable through this app.** |
...
⚠️ **Addendum to the addendum, same day, `2960d74`.** 79 minutes after the measurement above, `update()` was changed — an unrelated, owner-requested fix, not this spec's — to set `email_verified: 'true'` in the same `AdminUpdateUserAttributesCommand` as the new `email`. That removed the only application path that ever invoked this trigger source at all: the handler this section describes is correct, and was necessary at the moment it shipped, but the source it handles is now dormant, exactly as `VerifyUserAttribute` already was before this addendum was written. Nothing here is withdrawn — the break was real when measured, and the fix closed it while it was still live.
...
⚠️ **Consequence for T-4:** the `AdminCreateUser` branch and its use of `buildAttributeVerificationMessage` are **deleted**, not re-copied. At the time T-4 shipped, the handled set was `ForgotPassword` and `VerifyUserAttribute`. **⚠️ Corrected post-deploy (validation-report.md B-4, addendum above): the handled set is now exactly three** — `ForgotPassword`, `VerifyUserAttribute`, and `UpdateUserAttribute` (the actual admin-edit source). Everything else raises, naming the source. ⚠️ **Stale within the day (§5's addendum-to-the-addendum, above):** `2960d74` removed the app's only path to `UpdateUserAttribute`, so of these three only `ForgotPassword` is exercised by the running application today — `VerifyUserAttribute` and `UpdateUserAttribute` are both handled-but-unreached. The handled set itself does not shrink: a correct handler for a currently-unreached source costs nothing, per DD-2's own reasoning.
...
⚠️ **Step 4 — attempted 2026-09-23 and blocked; PERFORMED 2026-09-24. This paragraph records both, because the block is a finding in its own right.** First attempt: the deploying principal (`cognito_csicap`) got `AccessDenied` on both `cloudformation:CreateChangeSet` and `cloudformation:CreateStack`, for every stack name tried — a hard IAM boundary, not a judgment call. No stack was created. The weaker CLI-only rehearsal was **deliberately not substituted**: it does not exercise CloudFormation's `UpdateUserPool` composition, which is the whole risk being measured, so it would have produced a green result that measured nothing. A scoped grant (CloudFormation on `accelerate-tz-dev-rehearsal-*` only) was requested and granted, and the rehearsal ran on 2026-09-24 as `accelerate-tz-dev-rehearsal-t5`: pool deployed → live drift applied out-of-band and **verified landed** → template redeployed → full before/after diff → stack deleted. **Result: CloudFormation reset exactly the two (c)-class settings §6 predicted (`EmailConfiguration`, `AdminCreateUserConfig.InviteMessageTemplate`) and nothing else.** ⚠️ **Read that precisely: it measures the MECHANISM, not the (a) classification.** The rehearsal pool was built from this same template, so every omitted property already sat at the AWS default *by construction*, and only the two (c) settings were ever drifted away from it — so the (a) rows could not have done anything but survive, and their surviving discriminates nothing. **The two (c) resets are measured; the fourteen (a) rows — fifteen keys, since one row names two — remain reasoned.** ⚠️ *(Corrected 2026-09-25, validation-report.md D-1. This read "twenty-two," which is §6's total row count, not its (a)-subset — a total-for-subset inversion in its own right, but **not**, as this sentence previously claimed, "the same" defect the CLI-help footnote above already caught once. That one (§6, "seven of §6's rows," corrected from a first-draft "fifteen") was a **wrong-document substitution** — "fifteen" was `template.yaml`'s bullet count, a different artefact entirely, standing in for a count that was never about it — not a total standing in for its own subset. Two different defect species, not one recurring twice; the mischaracterization is fixed here rather than repeated a third time. Counted directly from §6's table as it now stands: 22 rows total — 14 carrying an explicit `(a)` label across 15 named keys, 2 × `(c)`, 4 "matches", 1 "not drift" (`UsernameAttributes`), 1 "not applicable" (the read-only metadata row). 14 + 2 + 4 + 1 + 1 = 22 rows. ⚠️ **The bullet count is deliberately not cited as corroboration here** — `template.yaml`'s comment block has 15 `(a)`-labelled bullets, but they name **16** keys (one bullet covers both `EmailVerificationMessage` and `EmailVerificationSubject`), and they include `UsernameAttributes`, which this table classifies as "not drift," not `(a)`. Two counts agreeing only in total is not agreement; the `UsernameAttributes` disagreement between this table and the template comment is real and is recorded here as an **open note**, not resolved by reclassifying either side.)* What would measure them: drift an (a) property away from its default (say `MfaConfiguration: ON`) and see whether a template-composed update resets it. Not done, and named here rather than left implied. Full evidence in `execution.md`'s T-5 entry. ⚠️ **And one thing nobody predicted:** redeploying an *unchanged* template returns `No updates are to be performed` — CloudFormation diffs template text, not live state, so the reset does not fire until a property actually changes. **T-6's `LambdaConfig` addition is the change that fires it.**
...
**But rollback is where round-1 C-5's second half bit, and it is still true:** removing the trigger resumes Cognito's own sending, now on `COGNITO_DEFAULT` — worse than the **pre-spec** SES behaviour ⚠️ *(re-tensed 2026-09-25, validation-report.md W-8 — this read "today's SES"; the pool has been on `COGNITO_DEFAULT`, not SES, since this spec's 2026-09-25 deploy, so "today" no longer names an SES state)*. Since SES is excluded permanently, **there is no rollback that restores current behaviour.** Rollback means self-service reset degrades to Cognito's shared sender until the trigger is restored.
...
⚠️ **Marked 2026-09-25 as the pre-decomposition estimate it was, not aligned to `tasks.md` (validation-report.md D-3).** `tasks.md` §4 is the decomposition of record and did not exist when round-1 C-12 asked for this list; rewriting the numbers above to match it after the fact would misrepresent this as having matched all along. Checked position-by-position against `tasks.md`, it disagrees in **6 of 7**: #1/#4 are `tasks.md`'s T-4/T-1, swapped; #3 bundles all three KMS policies as one task, but DD-2a (§4, decided after this list) splits them — T-3 delivers only the key-policy grant, the other two are T-4's, so no single task matches #3 either; #5 bundles `LambdaConfig` with the drift audit, but `tasks.md` splits that into T-5 (audit only) and T-6 (the trigger); #6 and #7 (TRD updates, the manual check) both fold into `tasks.md`'s single T-7. Only #2 ↔ T-2 (message templates) matches without qualification. Left as-is; `tasks.md` §4 governs.
...
`docs/trd/trd.md` §12.1's C4 Level-1 diagram, at the time this line was written, labelled the arrow from this system to the AWS Cognito box *"authenticates; self-service password-reset mail (COGNITO_DEFAULT)"*. That was false at the time (the live pool was still on SES; `COGNITO_DEFAULT` was only the *template's* value, per §6's (c)-class row) and would become false differently once this spec shipped. Correcting it is a `proposal.md` §14.4 success criterion that **went unscheduled across both previous designs** (`judgment.md` round-1 S-5, round-2 R2-9). It is **T-7** (`tasks.md`) — the manual live check's task, not T-6 (which only activates the trigger; §6, DD-4).

⚠️ *(Corrected 2026-09-25, validation-report.md D-2/D-3 — two errors in the paragraph above, fixed in place rather than left to compound.)*
- **D-2:** the paragraph previously quoted the arrow in the present tense, as if it still read that way. It doesn't: T-7's own commit (`d96f08b`) rewrote that exact arrow text as part of closing this task, replacing it with "authenticates; generates, expires & verifies reset codes." History established from `git show d96f08b -- docs/trd/trd.md`, not assumed: the diagram was edited (by this spec's own T-7); this citation simply never was. The bare line-number citation (`trd.md:288`) is also replaced above with the diagram element it names, per KZ-009 — line numbers drift, and this one already had (the arrow sits at line 286 now).
- **D-3:** the paragraph previously said "It is task 6." `tasks.md` T-6 is "Activate the trigger"; the TRD correction is T-7's second scope item, alongside the manual check this same sentence used to cite on its own. The error traces to this document's own §9 (above), whose numbered list puts "TRD updates" at position 6 and "the manual live check" at position 7 — two items `tasks.md` merged into one task, T-7. §9 is now annotated with the full count.
