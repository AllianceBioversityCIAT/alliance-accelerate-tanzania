...
**`design.md` §9/DD-7's original estimate: 8 groups, 18 records** (since corrected to 11 groups / 24 records, matching the measurement below — **C-14**, see the resolution marker at the end of this section). `design.md` DD-7 requires these listed **by organisation name and row** — organisation names are permitted (not PII); individual producer names are, and are excluded per DD-6/§5 above. A human decides; code never merges (DD-7).
...
**Finding — the design.md/DD-7 estimate of "8 groups, 18 records" undercounts against direct measurement.** The 11-group, 24-record figure above already exceeds the estimate using only the `Seed Company`↔`QDS`-tail correspondence `mapping.md` §4.5 had already described in prose — a fact that was measurable from that description alone, without opening the workbook, and this task's own predecessor apparently did not reconcile the two. Per this task's brief (*"if your measurement yields a different count... report the discrepancy as a finding — do not silently adopt either figure"*), this was reported for the Leader's adjudication. The closing sentence below was accurate when this task wrote it and is superseded by the Leader's subsequent adjudication, not retracted: *"`design.md` §9/DD-7 is not edited by this task."*

**Resolved 2026-08-05.** The Leader adjudicated this finding and the user approved the correction: `design.md` §9's measured-findings table and DD-7's Context line are now corrected to **11 groups, 24 records**, matching the measurement above — see `execution.md`'s T-10 finding — **C-14**. This resolves only the duplicate-candidate **count** discrepancy; no bucket total, yield figure, or the two lower-confidence pairs held outside the count (below) is affected. The 8-groups/18-records figure recorded above is the original estimate this measurement superseded, left standing as the audit trail — it is no longer `design.md`'s current figure.
...
- [x] §6's duplicate-candidate table is completed and measured (11 groups / 24 records, exceeding the original 8/18 estimate — resolved 2026-08-05, `design.md` corrected to match — **C-14**, see §6).
