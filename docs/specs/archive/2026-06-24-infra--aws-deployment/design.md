...
- **`DATABASE_URL`** is **composed** in this stack via `!Sub`, with the password supplied by a CloudFormation **dynamic reference** to Secrets Manager (never a plaintext template literal): e.g. `!Sub "mysql://${DbUser}:{{resolve:secretsmanager:${DbSecretArn}:SecretString:password}}@${RdsEndpoint}:3306/${DbName}?sslaccept=strict"`. Host/user/dbname come from the 10-data-auth outputs; the password is injected at deploy time (so the secret value never appears in the template or repo — NFR-2). TLS enforced via the connection string (`sslaccept=strict`) and Prisma's MySQL TLS. (Rationale: a fully pre-composed `url` key cannot live in the secret because the RDS host is unknown until the DB exists — see DD-4.)
...
### Decision: DD-4 — Secrets via Secrets Manager; DATABASE_URL composed with a dynamic-reference password
The 10-data-auth secret holds only `{username, password}` (`GenerateSecretString`); RDS consumes username/password via dynamic references. A pre-composed `url` key is **not** stored because the RDS host is unknown at secret-creation time (chicken-and-egg). Instead, `DATABASE_URL` is **composed** where the host is known: the backend Lambda env (T-3) via `!Sub` + `{{resolve:secretsmanager:...:SecretString:password}}`, and the migrate/seed script (T-6) in-process from the secret JSON + the RDS endpoint. The password is therefore never in a template, env file, or the repo (NFR-2). The dev machine pulls the same secret for `migrate deploy`. *Rejected:* (a) storing a composed `url` in the secret — impractical in pure CFN without a custom resource for the host; (b) plaintext template param / committed `.env` — violates NFR-2 and the `.env` hook.
