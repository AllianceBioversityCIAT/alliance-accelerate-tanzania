...
- AND IT MUST use `AcknowledgeDialog` and not `ConfirmDialog`: `frontend/CLAUDE.md` makes `AcknowledgeDialog` **required** before any submit that sets `consentStatus` to `GRANTED`, and `ConfirmDialog` is a destructive-confirm component while this is a publish action, not a delete
- **Corrected at T-16 — this scenario previously also cited `ConfirmDialog`'s `bg-danger` confirm button as a reason to prefer `AcknowledgeDialog`.** That was false: `AcknowledgeDialog.tsx:379` renders the confirm button with the identical class string as `ConfirmDialog.tsx:245`, both `bg-danger`. Choosing `AcknowledgeDialog` over `ConfirmDialog` does not avoid the red destructive button on this publish action — it was a live NFR-6 violation either way, escalated as `design.md` §12 R-13 rather than fixed by that correction. **R-13 was subsequently RESOLVED post-spec (2026-09-02):** `AcknowledgeDialog` gained a `tone` prop defaulting to `'danger'`, and this spec's approve call site passes `tone="primary"`. **NFR-6 now holds at the rendered DOM**, and an assertion pins it
...
| **NFR-11** | **Type fidelity of the audit-action contract (new).** The frontend audit-action union is exactly the backend enum — same members, same spelling, no widening to `string`. | A test derives or asserts the union against the backend's enum member list and fails on any divergence, **demonstrated to fail against the pre-change code** where `IMPORT` is missing. `frontend/CLAUDE.md` records that loosening such a union has FAILed reviews before. **Not literally met, documented at T-16 (A-62 posture, execution.md A-88):** the delivered test hardcodes the eight members frontend-side rather than deriving them from the backend's Prisma enum, so **backend-only enum drift — the exact defect this NFR exists to close — remains ungated.** `tasks.md` did not ask T-15 for the derivation, so this is a gap between this requirement and its decomposition, not a defect in T-15's work. Recorded here as an accepted gap rather than closed by a T-16 code change (this task carries no verification command); a real fix would either have the frontend Jest suite parse `schema.prisma` at run time, or add a type-level assertion, and is left as follow-up work, not committed to. |
...
| **DC-27** | **The frontend audit-action union or badge map drifts from the backend enum** (RA5 — live in the repo today for `IMPORT`) | NFR-11's test, **run against the pre-change code and shown to fail there**. Plus `npx tsc --noEmit`, since a total `Record<Action, …>` map turns a missing member into a compile error — note `next/jest` uses SWC and does **no** type checking, so `npm test` alone does not catch it. ⚠️ **During this spec's execution that command was baseline-red** (one pre-existing error, ticket A-73), so its exit status carried no information and T-15 diffed the **error set** instead; see `design.md` §7.5. A-73 was fixed in validation remediation, so the exit status is meaningful again — but check the baseline before trusting it (validation R12) |
...
| **DC-36** | **A total `Record<Union, string>` map (DD-21's `actionBadgeClasses`, and `RejectionReasonCode`'s label map from this spec's `RejectDialog`) does not catch the union being widened to `string`.** Added at T-16 (execution.md A-71/A-83). A total `Record` turns a **member added to the union with no map entry** into `TS2741` (a compile error) and a **map entry removed** into the same — but a field whose declared type is loosened from the literal union to plain `string` satisfies `Record<string, string>` trivially, so that specific regression compiles clean. This was believed closed by T-11's review (A-71) and found not to be by T-15's (A-83) | **Accepted risk, recorded — not gated.** No test in this spec asserts against type-widening; `frontend/CLAUDE.md`'s prior-FAIL precedent for loosened unions is a review-time check, not an automated one. A type-level exhaustiveness assertion (e.g. an `Exact<>`-style helper) would close it and is left as follow-up work |
...
**Defect classes:** DC-6, DC-7, DC-8, DC-13, DC-15, DC-23, DC-25 (inherited) · DC-27…DC-32 (new, gated) · DC-16, DC-24 (inherited, no gate) · DC-33, DC-34, DC-35, **DC-36** (new, no gate). *(DC-36 added to this index by validation R10 — it was entered in §8 at T-16 and swept into neither index, so both closed without it.)*
