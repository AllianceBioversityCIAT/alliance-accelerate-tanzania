...
- [x] **T-5** Sync the baseline docs, sweep stale values, and correct QA-11  (deps: T-2, T-3, T-4)
...
- [x] **T-6** Capture rendered evidence and run the human visual gate  (deps: T-2, T-3, T-4, T-5)
      ...
      **Done when:** gates green (suite gate per the note above); `git diff --stat` shows **only** `globals.css`, `tailwind.config.ts`, `lib/contrast*`, **`lib/dashboard/chart-tokens.ts`** (comment-only — see LF-1), the two comment-only component edits, and docs; captures exist for all **eight** surfaces at all three widths; **and the user has approved the visual result at the HITL gate.**
      > **Capture set widened to 8 surfaces (LF-1/LF-2, recorded in `execution.md`).** Add **`/dashboard` charts**. `chart-tokens.ts:44` puts `var(--color-warning)` at `CATEGORICAL_COLORS[7]`, consumed by `categoricalColor()` for Recharts series — so T-3's `#C9821B`→`#8F5E10` **changes an 8th-category chart series colour**, and `design.md` §5.4's list omits the dashboard entirely, meaning no capture would reveal it. One extra screenshot on an already-running stack is the only evidence that would show an unintended chart recolour. `chart-tokens.ts` is also added to the allowed-diff list above: FR-7's repo-wide per-value sweep **obliges** T-5 to correct its stale `#C9821B` comment, so a scope check that omitted it would FAIL on a file the spec itself mandates touching.
      > **Environment for this gate — Dev on AWS, by user decision (2026-08-07).** Captures are taken against the deployed CloudFront origin `https://d3idqvvg0xa1r7.cloudfront.net`, **not** localhost. Rationale: `npm run dev` is not the production CSS pipeline (Tailwind purges/minifies differently), so dev-mode output is weaker evidence for a *token* change than the production build — and the API's CORS `AllowedOrigin` is locked to the CloudFront origin (`infra/20-backend/template.yaml:200`), so the data-bearing surfaces (admin table, import preview, dashboard) cannot render with real data from any other origin without standing up a full local backend. Dev already carries seeded data. **The deploy is operator-run:** `infra/scripts/deploy-frontend.sh:31-32` states it "is NOT run by the SDD agent loop", so the user runs `./infra/scripts/deploy-frontend.sh` and the agent runs only the gate. That script's step 5 already issues `create-invalidation --paths "/*"`, which is what makes (a) below satisfiable at all on a CDN.
      > **Machine gate for disqualifier (a) — replaces "I hard reloaded" with a falsifiable check.** Fetch the CSS the origin is actually serving and assert the token set, rather than trusting that a reload happened:
      > ```
      > CF=https://d3idqvvg0xa1r7.cloudfront.net
      > CSS=$(curl -s "$CF/" | grep -oE '/_next/static/css/[a-z0-9]+\.css' | head -1)
      > curl -s "$CF$CSS" > /tmp/deployed.css
      > # MUST be present: #FBF9F6 #F4F0EA #2A2724 #8F5E10 #2A6E2D
      > # MUST be absent:  #F7F7F7 #333333 #C9821B(as --color-warning) #2F7D32
      > ```
