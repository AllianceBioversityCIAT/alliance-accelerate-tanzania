...
| `components/admin/ActorsTable.tsx` | **Source** column (chip) and **Consent** column (status chip + method caption). At `lg`+, the table scrolls horizontally within its `overflow-x-auto` container with the checkbox and Trader columns pinned `sticky` at the left edge, boundary marked by the `shadow-sticky-edge` token (`tailwind.config.ts`), and the Trader name width-clamped via an inner `<span>` (`docs/ux-ui/design.md` §9). Below `lg` (including `md`) the table does not render — a `lg:hidden` stacked-card list carries the same values instead. |
