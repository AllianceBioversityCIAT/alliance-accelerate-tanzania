...
- [x] **T-8** Surface source and consent in the admin actors table  (deps: T-3)
      ...
      **Not done if:** any color or radius is hardcoded instead of using `docs/ux-ui/design.md` §7 tokens (NFR-8), or the new columns' values are dropped or silently truncated on any surface — the stacked cards below `lg`, or the horizontally scrolling table at `lg`+. *(Corrected twice on 2026-08-04, tracking the two scope corrections below: originally "scrolled below `md`", which carried the false premise that a table exists below `md`; then `md`+, before the D-h measurement moved the table threshold to `lg`+.)*
      ...
      **Scope correction (2026-08-04, approved by JuanCode mid-execution, second pass):** the correction directly above set the table threshold at **`md`+**; the deliverables it lists are therefore superseded and kept only as a record of what was decided first. The D-h visual check measured `md` at **81% frozen / a 94px scrollable strip**, which rendered Consent as unreadable fragments even after the Trader-cell clamp. **The shipped threshold is `lg`+**: `hidden lg:block` table, `lg:hidden` cards, `TableSkeleton` moved to match. `requirements.md` FR-6 and `design.md` §5 each carry a matching second correction. Found by `/akili-validate` (`val-FR`) as **stale-completion-contract drift**: this line kept asserting `md`+ after the code and both other documents had moved, which is worse than the other staleness in this spec because a "Not done if" line *is* the completion contract.
