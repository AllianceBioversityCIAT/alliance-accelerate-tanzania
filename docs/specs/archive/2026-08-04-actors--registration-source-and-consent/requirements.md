...
>
> **The protected surface is covered regardless:** the public map and directory reach their data through `frontend/lib/api/actors.ts` → `GET /api/v1/actors` and `GET /api/v1/actors/{id}`, both of which this requirement asserts. When `/actors/geo` is eventually built, it inherits the assertion automatically — `pii-boundary.spec.ts` iterates the union of `PII_ALLOWLIST` and `NEVER_PUBLIC_FIELDS` rather than a hand-maintained list, so the new path inherits the **field list** automatically. *(Corrected 2026-08-04 by `/akili-validate`: this sentence previously said the new path "only needs to be added to the suite's path loop." There is **no** path loop — `pii-boundary.spec.ts` hand-writes one `describe` block per path with duplicated assertion bodies, so adding `/actors/geo` means copying a block, not appending to a list. The union-iteration half of the claim is true; the effort estimate was not.)*
>
