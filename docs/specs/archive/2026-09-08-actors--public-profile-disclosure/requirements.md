...
| D-1b | **The contact block reaches the list response** (and therefore the map, dashboard and CSV) — most likely as a silent consequence of widening the shared projection, not as a deliberate edit | FR-9's by-key **and** by-value assertion on `GET /api/v1/actors`, plus `csv.test.ts` naming the five fields | ✅ — must name the five fields explicitly, never assert a column count |
