...
|---|---|
| **Tasks** | **7** |
| **LOC** | **RETIRED as a tripwire — 2026-09-08, T-4 gate, user-approved.** History: 620 (Phase 2) → 635 (Phase 3) → ~900 (T-3 gate) → **retired**. Actuals: T-1 261/170 · T-2 63/35 · T-3 327/135 · T-4 314/190 — **965 lines against the ~900 it had just been raised to.** **A tripwire raised twice without any scope change is not measuring the thing it was meant to protect.** The task count never moved (7), no requirement grew, and the overrun is **56 % test code** — 313 of the 557 extra lines — with 244 in production files, much of which is the documentation that caught two defects no test could see (`isSamePoint`'s normalization contract, the 0×0 container). ⚠️ **An earlier version of this sentence said "the entire overrun is documentation". That was false**, caught at validation by recomputing prod (639 vs 395 est.) and test (553 vs 240 est.) from the shipped files. It also buried the real signal: **test-code estimation was off by 130 %**, which is the finding worth carrying to the retrospective. LOC was a reasonable proxy before execution; four tasks of evidence say the real cost here is **review rounds**, which remain a live gate. Recorded rather than raised a third time. |
| **Review rounds** | **16** — re-baselined 2026-09-08 after T-2, user-approved. The original **9** assumed reviews audit *code*. Across T-1 and T-2: **5 code-review rounds, 4 of them spent on the accuracy of prose rather than the correctness of code** (corrected at validation — this read "7 rounds, 6 of them", a figure never measured; recounting the ledger gives 5). The proportion that justified the re-baseline holds; the absolute number did not — T-2's code was accepted on attempt 1 and never changed again, while three rounds went into one docblock. Every FAIL was legitimate (a false claim in a module written to be read by the next implementer is a real defect, and one of them was caught before T-3 could inherit it), so the discipline is not the problem — the estimate was. LOC and task count both held: 323 of 635 at 2/7 tasks, so T-1's overrun was the test-dense outlier it looked like, not a uniform underestimate. |
