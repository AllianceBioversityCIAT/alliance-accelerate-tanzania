...
| **F-4** | **A + B** | SUGGESTION | "~43 kB gzip" is 41+2, each rounded down first. Measured: 42.7 kB JS + 2.7 kB CSS ≈ **44–45 kB**. Immaterial to the 116 kB bar *(that ceiling was later amended to **119 kB** at the T-6 gate, on measurement — see NFR-1; this row is left as written because the ledger is a frozen record)*; exactly the compounding-rounding KZ-005 warns about | **Fix** |
