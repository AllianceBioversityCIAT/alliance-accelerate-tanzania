...
- Status: **Done** — implemented, validated 2026-09-15 (three validators, FAIL → remediated). Two items remain open: Legal's unfilled contact fields, and the NFR-5 human render check.
...
> **Written before execution; kept as the record of the problem this spec was created to solve. Every present-tense claim below was true on 2026-09-15 at authoring and is false now — the spec succeeded.** For the shipped state see `execution.md` and `validation-report.md`.
...
| **D-5** | **OQ-1 resolved: English only.** No i18n is built. | No i18n exists in the repo. **No speculative `locale` field is added** — that half of the decision is correct and verified. ~~Zero-cost hedge: a future Swahili edition enters as `v2.0-en`/`v2.0-sw` with no redesign.~~ **That hedge was false and is withdrawn (validation, 2026-09-15).** `deriveCurrentEdition` returns the **last** element of the registry, and `CONSENT_POLICY_VERSION` and `CONSENT_POLICY_SECTIONS` both flow from it — appending `v2.0-en` then `v2.0-sw` would make Swahili the only served edition for everyone. Concurrent locale editions need a **selection mechanism**, which is exactly the single-current-edition derivation FR-1 scenario 3 mandates. A future locale therefore requires a redesign this spec does not build. |
...
| **D-7** | **Contact details live inside the legal prose**, not as a link to `/contact`. `ContactForm.tsx` is not modified. | Legal's text supplies its own contact block. **Correction (validation, 2026-09-15): that rationale is false at HEAD.** Every contact block in all three delivered documents is blank or `Insert …`, so the decision not to link `/contact` currently leaves a data subject told to contact nothing. The decision itself still stands — the filled values belong in Legal's prose, not in a link — but it is **blocked on CIAT supplying them**, and that blockage is now recorded in `docs/ux-ui/design.md` §4 and tracked by the per-document unfilled-field inventory tests. |
| **D-9** | **The contact-channel facts stay on `/privacy`, authored by engineering, not by Legal.** Legal's *"Information We Collect"* covers what a contact submission collects; the three remaining facts — who receives it, that it is relayed and not stored, that submitting is not consent to publish — are carried as **one titled section with three sub-headings**, *Messages sent through the contact form*, placed **immediately before Legal's "Contact Us"** — i.e. inside Legal's document, not after it. *(Amended 2026-09-15. It originally read "a short section after Legal's block" and described three separate sections; the product owner objected on 2026-09-15 that bare engineering sections interleaved with Legal's read as part of Legal's document, and the restructure into one contained section followed. Validation found the decision text still describing the pre-restructure arrangement.)* | Resolves OQ-A (2026-09-15). The approved policy covers the collection half; the other three are system behaviour, not legal commitment, so asking Legal to author them would be asking the wrong party. Carrying the four superseded sections in wholesale was rejected: three of them are now redundant with Legal's text. |
| **D-10** | **`/privacy` does not name Google. The recipient disclosure lives only on `/cookies`.** Legal's Cookies section is carried verbatim, unamended, including its *"does not use cookies to collect personal information"* sentence. | Resolves OQ-B (2026-09-15). D-4's division applied consistently: the policy states the commitment, the notice states the fact. The site as a whole discloses the recipient and the IP-derived geolocation on `/cookies`, which is where the consent banner sends every visitor it asks for consent. **Recorded as an accepted divergence, not a reconciled one** — Legal's sentence and `/cookies`' inventory are in tension, and the resolution is placement, not amendment. **Validation finding (2026-09-15), recorded rather than glossed:** §8's defect class 8 and `design.md` §9 both promise the two NFR-4 findings go back to Legal *"before the text is frozen"*. They did not, and the text **is** frozen at `v1.0`. The spec's only ungated defect class therefore shipped with its only named substitute unperformed. The divergence is wider than D-10 first stated: `/privacy` publishes Legal's four other cookie purposes — administrator authentication, fraud detection, performance, troubleshooting — which are the **same four** FR-3 forbids `/cookies` from claiming, while `/cookies` makes the exclusive present-tense claim that GA4's are the only cookies the site sets. Two published documents of one site, and neither tells a reader which governs. **Carried to the product owner as an open item, not closed here.** |
...
- ~~**Blocking for FR-2, FR-4, FR-5:** the approved texts.~~ **Delivered and landed 2026-09-15.** What remains outstanding is narrower and is Legal's to close: the unfilled fields inside those texts (`Insert Date`, `Insert CIAT Legal Entity`, the contact blocks). Tracked by the per-document inventory tests; until they are filled, `/terms` and `/privacy` are **not publishable**. Every other requirement is independently executable, and the task order in `tasks.md` reflects that.
