...
**Budget: keep `## Active Lessons` under ~12 KB.** *(Raised from ~8 KB on 2026-09-08, at the cap, applying this file's own advice — "prefer raising the budget" — rather than retiring rows to make room. Six rows qualified as `Applied`-and-never-recurred and were **not** retired. **KZ-011 in particular must not be**: it is the lesson that every claim about a third party, the harness or the codebase must cite where it was verified, and `enhancement/map-coordinate-picker` violated it five times while it sat in the table marked `Applied`. Retiring it as "the standardization held" would be precisely the failure this preamble warns about — mistaking a row that is load-bearing for one that has become a pointer.)* This was a 10-**row** cap until
...
| KZ-002 | A presence-assertion (class/config/attribute exists) is not a behavioral proof — it must record what it cannot prove, and a property the harness cannot evaluate is **not covered**. **Recurrence ×2 (2026-08-05): extended to documents** — a procedure carrying every required clause can still be unexecutable. **Recurrence ×3 (2026-08-08): now in the verify command itself** — a gate that greps generated output must be run against the pre-change state and shown to differ; a gate that cannot fail is not a gate. **Recurrence ×4 (2026-09-01): all 7 FAILs in one spec were this class.** New forms: a suite passing 11/11 while blind to the very defect the task existed to prevent (deleting the guarded flag left every test green); an error-path test unable to fail on 3 of the 4 properties it named, because the handler runs in a microtask its synchronous assertions never reach; and — new — **a gate whose pass state depends on where an arbitrary sample lands**, which measures nothing even at zero failures **Recurrence ×5 (2026-09-08): seven instances in one spec, and the locus IS the finding — every one sat in a *verification line*, never in a requirement and never in design intent.** That spec's content survived two judgment rounds and fifteen reviews; the gates authored alongside it failed seven times. Three were caught only because a Reviewer read *one task ahead* — including a `Verify` command (`npx eslint …`) that **could not start at all** in that package | actors/registration-source-and-consent · import-export/partner-profile-onboarding · enhancement/form-elevation-ux · enhancement/usage-analytics · enhancement/map-coordinate-picker | High | Product + Methodology | `docs/specs/general-setup/task.md` § Testing & Verification | Applied |
...
| KZ-004 | A correction is not applied when its cited site is fixed — grep the superseded value across every document, and mark as resolved everything that *quotes* the corrected figure, in the same change. **Recurrence (2026-09-01): the sweep applies *inside a single rework brief*, not only across documents** — folding two items where one's text asserts a boundary the other's change moves produces a self-falsifying instruction the Implementer will follow correctly **Recurrence ×3 (2026-09-08): a correction applied to one cited site and not to its siblings.** A coverage-table row demoted at one task's gate left the two sibling rows found a task earlier still claiming a test that did not exist — and a sweep that fixed a superseded budget figure in `tasks.md` missed the identical line in `execution.md`'s own header | import-export/partner-profile-onboarding · enhancement/usage-analytics · enhancement/map-coordinate-picker | High | Product + Methodology | `.agents/leader.md` § Applying a correction | Applied |
| KZ-005 | Every numeric claim must be cross-checked against narrative prose in the same spec before publication; a count that contradicts a sentence in a sibling document is a defect detectable without re-measuring. **Recurrence ×2 (2026-09-01), both by the Leader** — a 68-line file-length error that **suppressed a third budget breach**, and "20 rounds / 9 FAILs" reported when the log recorded 15 and 7. **Severity raised Medium → High: an unmeasured breach disarms the budget tripwire retroactively** **Recurrence ×3 (2026-09-08): extended to the Leader's own ledger, the one document no Reviewer audits.** Review-round counts reported from recall across three consecutive gates, drifting upward (real 5 → reported 7; real 10 → reported 12–14), with a **budget escalated on the inflated figure** — then the same basis re-asserted as *"measurable and true"* in the very paragraph admitting the counts were kept by memory. KZ-005 surviving its own correction | import-export/partner-profile-onboarding · enhancement/usage-analytics · enhancement/map-coordinate-picker | **High** | Product + Methodology | `docs/specs/general-setup/requirements.md` § Writing Standards · `.agents/leader.md` § Reporting To The User | Applied |
...
| KZ-008 | **An assertion about an artefact is a defect when the artefact does not bear it** — and it recurs at every level of its own correction. **Recurrence ×2 (2026-08-08): now extended to evidence artefacts** — capture manifests, README provenance claims, and `execution.md` status tables, not only code comments. **Recurrence ×3 (2026-09-01): 11 instances in one spec, 4 blocking.** Two were *corrections that introduced a fresh false claim* — including one where the Reviewer's own suggested wording, transcribed verbatim, was falsified by a second fix folded into the same brief **Recurrence ×4 (2026-09-08): five instances in one spec, all authored by the Leader, and the mechanism is now nameable — reading *about* an artefact feels like reading it.** A `*`-quantified regex's output read as a count; a compiler message naming an alias read as its expansion; a symbol written from memory; *"git history is permanent"* applied to files never committed; a guide claiming both consumers import the same constants when one imports three. **None was caught by re-reading; every one by measuring** | actors/public-self-registration · enhancement/form-elevation-ux · enhancement/usage-analytics · enhancement/map-coordinate-picker | High | Product + Methodology | `docs/specs/general-setup/task.md` § Testing & Verification | Applied |
...
| KZ-012 | **`author ≠ auditor` holds on reading, collapses on execution** — every red/green in a completion report was produced by the agent that wrote the code. The Reviewer must mark the boundary of what reading established: an A/B whose control cannot be inspected is uncorroborated; a count not recomputable from the diff is reported, not verified **Recurrence (2026-09-08): extended to test *authorship*.** `author ≠ auditor` was structurally enforced on review by the agent wrappers and held throughout — and never applied to tests: `/akili-test` was not run, so every test was written by the agent that wrote the code it tests. Four non-discriminating suites resulted; three were caught anyway by the demonstrated-falsifier rule (KZ-002) | enhancement/usage-analytics · enhancement/map-coordinate-picker | High | Methodology | `.agents/reviewer.md` § You Audit Reading, Not Execution | Applied |
