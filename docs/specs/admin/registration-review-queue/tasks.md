...
- [x] **T-12 Queue page + `RegistrationsTable` + sidebar entry** (deps: T-11)
      ...
      Clause sweep: `BUT it must NOT present a segment for AWAITING_APPLICANT/WITHDRAWN` and `AND IT MUST NOT present a "No email" flag` → both are **absence** assertions, which is the direction that matters; both can only prove absence and that is recorded. `AND IT MUST keep filter, **sort** and page state in the URL` → assert `router` receives the synced query for each of them. *(The word `sort` was dropped between `requirements.md:125` and this task, and the corresponding assertion was never written — stripping the `sort` push from `page.tsx` left the whole suite green while the control stopped being shareable. Restored by validation R16.)* `AND IT MUST inherit the shell's gate` → `NavItem` gains no role field; assert its shape is unchanged.
...
- [x] **T-14 Decision surfaces — approve via `AcknowledgeDialog`, `RejectDialog`, dismissal wiring** (deps: T-12, T-13)
...
- [x] **T-15 Audit-action taxonomy end-to-end (FR-16)** (deps: T-1)
...
- [x] **T-16 Amend the baseline documents** (deps: T-10, T-14, T-15)
      ...
      **⚠️ CARRIED FORWARD FROM T-14'S REVIEW — a spec defect in TWO documents (`execution.md` T-14).** `design.md` §7.4 rejects `ConfirmDialog` partly because *"it hardcodes its confirm button as `bg-danger` — a red destructive button on a publish action"* — but **`AcknowledgeDialog.tsx:379` is byte-identical to `ConfirmDialog.tsx:245`**. `requirements.md` FR-12 scenario 3 carries the same false clause, where the word ***"additionally"*** is what makes it false. **Sweep BOTH documents or KZ-004 fires.** Three things this task must carry: (1) the prose fix in both; (2) **the visible outcome escalated to the user — the prose fix alone leaves a red destructive button on the system's only publish action, converting a live NFR-6 violation into a documented one.** Cheapest remediation to offer: a `tone?: 'danger' | 'primary'` prop on `AcknowledgeDialog` defaulting to `'danger'`, so the three existing call sites render byte-identically; (3) a note that the passing token test is scoped to the *trigger* buttons and does **not** prove NFR-6 for the dialog confirm.
      **Also carried (`execution.md` A-83, and the running tally):** A-71's promised protection does not exist — a total `Record` catches a member added or removed, but **not widening to `string`**, which was its actual concern. And this spec's stated verification has now proven imprecise **six times**: T-1 (vacuous falsifying input), T-4 (clause misassigned to a route with no `:id`), T-7 (a mutation its own test cannot detect), T-9 (`Verify` naming a suite that cannot exercise the leak), T-14 (props-preserving swap is rendering-indistinguishable), A-83. **Record the pattern, not just the instances.**
      Scope: TRD §2 (module responsibility), §4 (+5 endpoints), §8 (adjudication authority), §12.5 (**ADR-012**, drafted in `design.md` §9), §13 (QA-3, and resolve B3 — either widen QA-12's actor text to cover admin-gated PII routes or cite QA-3 alone). `docs/ux-ui/design.md` §2 IA (+2 routes), §4 screen inventory (+2 screens), §5 admin shell nav.
      Traces: `requirements.md` §4.2 · `design.md` §9 ADR-012 · `judgment.md` B3
      ...
      Done when: all sections amended, ADR-012 entered with status `Accepted`, B3 resolved one way or the other and the choice recorded.
      **Re-measure the budget at this task's close.**
...
| # | Gap | Substitute |
|---|---|---|
| **DC-16** | Contrast, focus order, focus visibility on both new screens. **RESOLVED — do not re-check the pairing named here.** T-12's review flagged the `PENDING_REVIEW` chip as `bg-border text-muted` at **≈4.45:1**, under the 4.5:1 AA floor and seen by no gate (`border` is not one of `contrast.test.ts`'s nine grounds). That pairing no longer exists on either new screen: T-14 extracted the status vocabulary to `frontend/lib/content/registration-status.ts`, where `PENDING_REVIEW` is now `bg-surface-alt text-warning` (**4.90:1**, measured), and A-76 closed it by moving the chip **from ungated to gated**. All three producible statuses are now covered automatically — `frontend/lib/contrast.test.ts` lists `surface-alt` among `warning`'s reachable grounds and asserts every reachable pair at ≥ 4.5:1; `highlight-tint`/`success` (APPROVED) and `danger-soft`/`danger` (REJECTED) likewise. **Live trap for chunk 4:** `REGISTRATION_STATUS_BADGE_CLASSES` still carries `bg-border text-muted` for `AWAITING_APPLICANT`/`WITHDRAWN`, which D-7 keeps unreachable here — the moment `admin/registration-info-requests` makes them reachable, those two pairings ship ungated. | **Human check at the Phase-3 HITL pause**, real browser. `jest-axe` disables `cat.color` entirely. KZ-003: these components take plain props — **never defer this on auth grounds.** |
