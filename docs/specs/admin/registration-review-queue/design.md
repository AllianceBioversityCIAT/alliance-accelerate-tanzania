...
**Analytics exclusion is inherited for free.** `enhancement/usage-analytics` implements admin exclusion as **layout placement, not a pathname allowlist**: the analytics stack is mounted in `app/(public)/layout.tsx`, so no `(admin)` route can reach it regardless of its path. Any new route under `(admin)` is therefore excluded automatically. **No task, no test change.**

Its gates, read at `app/(admin)/analytics-exclusion.test.tsx` rather than assumed — three of them, none a source sweep of the admin layout:

| Gate | What it asserts |
|---|---|
| `AdminLayout` render block (`:219`–`:318`) | With a **granted** consent record seeded, the admin shell renders **zero** GA4 script elements, **no** consent banner, and leaves `gtag`/`dataLayer` `undefined`. Anti-vacuity guarded at `:262`, which proves the shell really rendered rather than redirecting |
| Root layout source sweep (`:330`–`:339`) | `app/layout.tsx` carries no `ConsentProvider`, `ConsentBanner`, `GoogleAnalytics`, or `/analytics/i` reference — the mount cannot be hoisted above the route groups without reddening this |
| `(public)/layout.tsx` source sweep (`:349`–`:356`) | No `usePathname`, no `pathname`, not even `'use client'` — so the mount is unconditional **by construction**, which is what makes the mechanism placement rather than an allowlist |

**Correction (validation, 2026-09-02).** This paragraph previously asserted that the gate *"asserts the admin layout contains no `usePathname` reference"*. That was false twice over: the `usePathname` sweep is over the **public** layout, and the admin layout does import and use `usePathname` (`app/(admin)/layout.tsx:29,77`) to close the mobile menu on navigation. The conclusion drawn was nonetheless correct, and the real gate above is **stronger** than the one described — behavioural rather than textual. The sentence was found by an independent auditor during `/akili-validate`, in text this spec's own Leader wrote, and it ended with the words *"rather than assumed"*: the defect class this spec spent sixteen tasks hunting, committed in the act of claiming immunity from it.
...
| **usage-analytics L-1** (nothing verifies the spec is *true*) | §7.2 refuses to assert a breakpoint from reasoning and makes measuring it an obligation; §7.1's analytics-exclusion claim was **not** in fact verified against the test file despite saying so, and validation caught it — the lesson's own instance became its counter-example (corrected 2026-09-02; see §7.1); the sidebar's `NavItem` shape was read before being described |
