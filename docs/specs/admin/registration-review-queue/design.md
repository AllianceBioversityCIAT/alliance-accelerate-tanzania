...
| **R-13** | **`AcknowledgeDialog`'s confirm button is `bg-danger` on the system's only publish action** — a red destructive button styling an act that grants consent and publishes PII, contradicting this project's rule that `danger` means destructive (§7.4, corrected at T-16). Escalated to the user as an open item, not fixed in this spec. | **Cheapest remediation, named but not implemented here:** a `tone?: 'danger' \| 'primary'` prop on `AcknowledgeDialog` defaulting to `'danger'`, so the three existing call sites (bulk unlock, import commit, single-actor form) render byte-identically and only this spec's new call site passes `tone="primary"`. Out of scope for T-16 (documentary task, no verification command); requires a follow-up implementation task and a visual/token re-check. |
