...
- [x] **T-7** Surface the send status, and fix the copy this feature falsifies  (deps: T-6)
      ...
      *(`page.tsx` added during execution. `CredentialHandoff` has **two** call sites, not one: `CreateUserDialog.tsx:192` renders it for the create flow and `page.tsx:497` renders it for the **reset** flow — the flow T-5 made email. Making `emailSent` a required prop breaks `tsc` at `page.tsx` unless that call site passes it, and `page.tsx` must first capture the field from `resetUserPassword()`'s result into its `resetHandoff` state. The original Files list named neither the call site nor the state.)*
      ...
      ⚠️ **Pre-existing violation inside the element you are editing — measured 2026-09-21.** `CredentialHandoff.tsx:106`'s warning paragraph already carries `border-warning/40` and `bg-warning/10`, and **both emit no CSS at all**: the box renders with no background tint and a default-coloured border. Verified by compiling the classes directly — `bg-warning` and `border-warning` emit normally, `bg-warning/10` and `border-warning/40` emit nothing, and a control token declared as `rgb(var(--x) / <alpha-value>)` emits correctly. Cause: every token in `tailwind.config.ts` is a bare `var(--color-*)` and **no token uses the `<alpha-value>` form**. Fix these two while rewriting that paragraph (use `opacity-*` or a solid token); the same pattern appears at **32 sites repo-wide**, which is NOT this task's job.
