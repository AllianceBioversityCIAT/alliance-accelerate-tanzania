...
⚠️ **One finding beyond this table, discovered by T-5's audit and explicitly out of its scope to fix:** `aws cognito-idp update-user-pool help` states `VerificationMessageTemplate`'s (and `InviteMessageTemplate`'s) `EmailMessage`/`EmailSubject` are "allowed only if the value of EmailSendingAccount is DEVELOPER." This resource sets both `EmailConfiguration.EmailSendingAccount: COGNITO_DEFAULT` **and** a branded `VerificationMessageTemplate.EmailMessage`/`EmailSubject` (T-4's content, already reviewed and merged) — a combination `UpdateUserPool` may reject outright the first time it is actually called with both present. This predates T-5, is not T-5's scope to fix, and could not be confirmed empirically because the rehearsal that would have tested it did not run (see T-5's completion report). Routed to whoever owns T-6, whose deploy is the first one that would ever exercise this call.
...
⚠️ **Step 4, attempted 2026-09-23 (T-5), blocked — recorded here rather than left silent.** The deploying IAM principal available to this task (`cognito_csicap`, the same identity `CustomEmailSenderKmsGrantPrincipalArn` names above) received `AccessDenied` on both `cloudformation:CreateChangeSet` and `cloudformation:CreateStack` for **every** stack name tried, including one matching the `accelerate-tz-dev-` prefix the three real stacks share — not a judgment call, a hard IAM boundary: this identity can act on the three named stacks and cannot create a new one under any CloudFormation API path. No stack was created (confirmed via `describe-stacks`, `ValidationError: … does not exist`, both names) and nothing needed deleting. The rehearsal remains **not performed** — see T-5's completion report for the full evidence and the options for actually running it (broader/operator credentials, or a scoped IAM grant for future rehearsals of this shape).
