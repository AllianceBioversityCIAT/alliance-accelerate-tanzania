...
> ⚠️ **Post-hoc addendum, added later — does not alter the row above** (this document's own header marks the target "immutable at judgment time"; C-6's text is left exactly as both judges wrote and confirmed it). **C-6's cited `triggerSource` name was wrong**, discovered post-deploy by a live DEV measurement (`validation-report.md` B-4, `design.md` §5's own addendum). C-6 quotes `proposal.md` §2.3's *"the live edge … already named"* — that name was `CustomEmailSender_VerifyUserAttribute`; the measured source is `CustomEmailSender_UpdateUserAttribute`. **The finding's substance is unaffected**: both judges correctly identified that the admin-edit mechanism reaches this trigger and that raising on it breaks a shipped feature, and that is exactly what happened once the fix landed the wrong mapping — the judges were right about the mechanism on the same wrong name every other document inherited from `proposal.md` §2.3, because a `triggerSource` string is exactly the kind of detail a blind adversarial read has no independent way to re-derive from the design text alone; it can only be checked against Cognito, which none of these documents did until deploy.
