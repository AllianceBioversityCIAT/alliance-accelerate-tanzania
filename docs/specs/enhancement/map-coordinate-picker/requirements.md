...
| **D-5a** | **Dynamic split broken** — Leaflet static-imported into the picker's wrapper, moving ~44 kB gzip into the initial bundle | `npm run build` route table, compared against **M-1 (113 kB)** | **Yes** — a top-level `import 'leaflet'` in `CoordinatePicker.tsx` breaches the 116 kB bar |
...
|---|---|---|
| **NFR-1** | `/register` First Load JS MUST NOT exceed **116 kB** (M-1's 113 kB + ≤3 kB for the picker's own non-Leaflet code and the reveal control) | `cd frontend && npm run build` route table (**D-5a**). **Disqualifier:** if the build ran against a dirty `.next/` or a different dependency tree than M-1's, the number is not comparable — rebuild clean or report the run as inconclusive. **This gate does not cover FR-7 — see NFR-1b** |
...
| NFR-1 | `/register` First Load JS ≤ 116 kB | D-5a |
