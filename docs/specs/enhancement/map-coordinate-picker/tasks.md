...
- [ ] **T-2  Extract map constants — `components/map/map-constants.ts`**  (deps: none)
...
- [ ] **T-3  Leaflet shell — `components/map/CoordinatePickerMap.tsx`**  (deps: T-1, T-2)
      ...
      **Verify:** `cd frontend && npm run build && npx eslint "components/map/CoordinatePickerMap.tsx" --quiet` **plus** `grep -nE '#[0-9a-fA-F]{3,8}|rgb\(|bg-\[' frontend/components/map/CoordinatePickerMap.tsx` returning nothing.
...
- `npm run lint` in `backend/` mutates files; this spec is frontend-only, but use the `npx eslint … --quiet` form when verifying a diff.
