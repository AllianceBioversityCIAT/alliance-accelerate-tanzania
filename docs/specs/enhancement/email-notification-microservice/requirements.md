...
| **D-I** | **Closed 2026-09-17 — was: the container freezes during the fire-and-forget receipt send, dropping it after its `202`** (⚠️ *the `202` here is a separate, pre-existing error — this endpoint returns `201`, not `202` — reported for the Leader's separate sweep, not corrected in this pass*) | ✅ **Automated gate exists.** ⚠️ *Corrected 2026-09-17, round 6: this cell said "No automated gate", falsified by this same fix — the receipt is now AWAITED (`dispatchReceiptEmail`, no longer `void … .catch()`), and `registrations.service.spec.ts` carries a test proving `submitRegistration()` does not settle before the send does.* The residual `callbackWaitsForEmptyEventLoop` dependency this row described no longer exists for this path. | **Substitute, narrowed:** T-9 must still observe ACTUAL receipt delivery (not merely that the send was awaited) — the only evidence for D-G and D-H, per `design.md` §9's D-I row. |
...
- **Acceptance criteria:**
  ...
  - **BUT it must NOT** silently leave a document asserting a premise this change withdrew (KZ-004). The withdrawn-premise set is **at least seven sites**, not one — behavior stays unchanged at every one; only the stated rationale is corrected:
    ...
    5. `src/lambda.ts` — claims approval/rejection are "still dispatched fire-and-forget by design". They have been **awaited** since `fix/otp-mail-lambda-freeze`; ⚠️ *corrected 2026-09-17, round 6: this item said "only the receipt is not" — as of D-I (2026-09-17), the receipt is awaited too, and `lambda.ts` was corrected in that same fix.* The conclusion (the flag is load-bearing) survived the first correction, then stopped holding at all once the second one landed — `lambda.ts` now says the flag is load-bearing for zero paths
