...
| `receipt` | `RegistrationsService.dispatchReceiptEmail` | **Awaited** (D-I, closed 2026-09-17 — was fire-and-forget, `void … .catch()`) | None — this path runs after OTP verification, so there is no enumeration oracle left for a floor to protect |

All five awaits landed across two fixes: `fix/otp-mail-lambda-freeze` (2026-09-03) for the first four, and D-I's closure (2026-09-17, this revision) for the receipt. Both fixed the same class of **production incident**: Lambda froze the container the instant the response settled and dropped the in-flight send, with its `.catch()` never running.

⚠️ **A stale claim in the file our design depends on — corrected in two passes.** `src/lambda.ts` used to state the flag stays load-bearing because *"`AdminRegistrationsService`'s approval/rejection notices and `RegistrationsService.submitRegistration`'s receipt email are still dispatched fire-and-forget by design (DD-9)"*. Approval and rejection were **awaited** by the same 2026-09-03 commit that introduced that sentence, without updating it — so the conclusion (the flag is load-bearing) survived while its stated basis was two-thirds wrong. FR-6's withdrawn-premise sweep owned that first correction (§11). **The remaining third closed 2026-09-17 (D-I, §9):** the receipt path is now awaited too, observed in production exactly the way the OTP bug was (CloudWatch: a `mail send attempt kind=receipt` line with zero matching outcome line, intermittently). `src/lambda.ts`'s comment has been rewritten again to say what is now true — the flag is load-bearing for **zero** paths — and is left SET regardless, as a harmless backstop, not removed.
...
| `src/lambda.ts` | ✅ **Done (D-I, 2026-09-17).** The stale fire-and-forget claim (§3.1) is corrected — twice, in fact: once for approval/rejection, then again for the receipt when D-I found the flag's remaining basis was also stale. The comment now says the flag is load-bearing for zero paths |
...
- **Decision — Option A (product owner, 2026-09-15): bound *every* term, then compose the floor from the bounds.**
  ...
  **Invariant: `PRESEND_ALLOWANCE + LOCK_WAIT + SEND_TIMEOUT ≤ FLOOR`**, with **all four quantities in that inequality enforced at runtime** — the three summands plus `FLOOR` itself, none of them aspirational. The previous revision enforced only one of the (then three) and called the result structural. ⚠️ **Completed 2026-09-17 (D-I review, F4): `LOCK_WAIT` was missing from this sum.** `MicroserviceMailTransport.send` acquires the mutex BEFORE opening the `try` that races the rest of the send against `SEND_TIMEOUT`, so the lock wait is ADDITIVE to `SEND_TIMEOUT` from a caller's point of view, not covered by it (§12.1's `MAIL_SEND_TIMEOUT_MS` row and §12.2's heading both said otherwise; both are corrected). Omitting it left the floor under-counting the transport's real worst case by exactly `LOCK_WAIT`'s value — this is DD-10 applied to a term it missed, not a change to the decision.
...
- **Gate — corrected 2026-09-17 (Reviewer, round 4): there is no falsification test over the inequality, and there cannot be one.** `VERIFICATION_CODE_RESPONSE_FLOOR_MS` is DEFINED as the sum of the three SUMMANDS above (not counting `FLOOR` itself, the fourth quantity the Context paragraph's "all four" counts), so raising any summand raises the floor with it — the inequality is `x ≤ x` by construction, untestable by design, and a test asserting it would be exactly the green-gate-that-cannot-fail shape this decision exists to close (KZ-002; `registrations.service.spec.ts`'s own comment on this point). **The real gate is a pin on the live computed constant against §12.1's value** (`registrations.service.spec.ts` — value not restated here; five lines up this same section states the rule this literal would have broken) — it reddens if any constituent constant drifts **while the floor stays composed**, without the pin being updated to match. What that qualifier excludes: replacing the sum with a hardcoded literal keeps the pin green too — the composition itself is held by reading, not by test (see that spec file's own comment). **What neither the composition nor the pin covers — and what nothing here can gate — is a term that exists inside the timed window but was never named in the sum.** That is F4's own defect: `MAIL_LOCK_WAIT_TIMEOUT_MS` was real, timed, and absent from the composition until 2026-09-17. Catching a missing term is a reading task, not a test.
...
- **Both transports were bounded in Phase A, which is why the floor was not gratuitous then.** ⚠️ *Marked Phase-A-historical (2026-09-17, rounds 5–6 — round 6 caught the bolded lead itself still reading present tense after round 5's body-text fix) — unlike §12.1's `MAIL_SEND_TIMEOUT_MS` row (F9), this sentence is already scoped, in its own words, to a decision made while SES still existed ("unbounded **today**" meant Phase A's today). T-10 deleted `ses-mail.transport.ts` in Phase B; rewriting this paragraph to the present tense would erase the actual Phase A reasoning (why the floor was worth raising while a second, unbounded transport was still live) rather than correct a live claim.* Judgment round 3 found that landing a higher floor while still on SES would tax every applicant for a property the SES path does not enforce — `ses-mail.transport.ts` constructed `new SESClient({ region })` with no `requestHandler`, no `requestTimeout` and SDK-default retries, i.e. unbounded at Phase A time. Phase A therefore also gave the SES client the same `SEND_TIMEOUT`. The floor then held under whichever transport was active, and a pre-existing hole closed on the way past.
...
- **Decision — mutex, with explicit scope and release.** A module-scope async mutex is held **from probe/acquire through the confirm** — the wide scope, chosen deliberately: the narrow "serialise acquire/invalidate only" scope does not deliver the `'return'` attribution below, and round 3 found revision 3 asserting both scopes in consecutive paragraphs. ⚠️ **Premise corrected (D-I review, F1/F2, 2026-09-17): the receipt kind is no longer dispatched `void … .catch()` — it is AWAITED as of D-I (§3.1, §9), the same as every other kind.** The original rationale ("a receipt publish can be in flight while a concurrent request invalidates the pair") no longer describes a live path. **The mutex is retained regardless**, on grounds independent of any dispatch being unawaited: the `'return'`-attribution rationale below, which does not depend on this premise, plus future-proofing against any dispatch path that becomes unawaited later.
...
- **Cost, stated — premise corrected (F1/F2).** Waiting for the lock is charged to the caller. Previously: *"a concurrent `void`-dispatched receipt can therefore delay an awaited contact or OTP send."* Now that the receipt is AWAITED too, the accurate statement is: **any two concurrent sends can delay each other** — the mutex serialises ALL sends, not only a formerly-unawaited one. `ReservedConcurrentExecutions: 5` bounds this to five single-threaded containers, and `MAIL_LOCK_WAIT_TIMEOUT_MS` (§12) bounds the wait itself — a send that cannot acquire the lock in time fails having never touched the broker, which is reported distinctly from a broker failure so the operator can tell them apart.
...
| **D-I** — freeze during the **fire-and-forget receipt** send | **Closed 2026-09-17** (was High) | **Observed in production, not merely theorised.** CloudWatch, `/aws/lambda/accelerate-tz-dev-backend-api`, 2026-09-17 15:21:54: a `mail send attempt kind=receipt reference=REG-2026-0006` line with **zero** matching outcome line — the same attempt-without-outcome signature that caught the OTP bug — and intermittently, alongside sibling references (`REG-2026-0002`, `REG-2026-0003`) that DID log `status=sent`: the signature of a race against container freeze, not a deterministic failure. Closed by AWAITING `dispatchReceiptEmail` (product owner decision, 2026-09-17), bounded by the transport's existing `MAIL_SEND_TIMEOUT_MS` (§12) — no second timeout added, and deliberately no constant-time floor (this path runs after OTP verification; see `registrations.service.ts`'s updated class doc). `dispatchReceiptEmail` no longer depends on `callbackWaitsForEmptyEventLoop` at all — **the freeze-drop mechanism is closed by construction; T-9 must still observe actual receipt delivery, which remains the only evidence for D-G and D-H** |
...
| Queue is PROD, not DEV → unmarked real mail | ~~High~~ **Closed** | ✅ **Closed — on a corrected reason.** There is **only a PROD queue**; the `TEST - ` subject prefix follows the credential's environment, not the queue's name, and the issued key is non-PROD — verified empirically: all five received emails carried `TEST - ` in the subject (`requirements.md` DEP-5, product owner, 2026-09-16). T-9 unblocked |
...
| `MAIL_SEND_TIMEOUT_MS` | **3000** | The transport — ⚠️ *corrected (F9): said "both implementations"; T-10 deleted `ses-mail.transport.ts`, so `MicroserviceMailTransport` is now the one real-sending transport this bounds (`registrations.service.ts`'s DD-10 text already says so — this cell had not been swept)* | Probe + any reconnect + publish + confirm — ⚠️ *corrected (F4): does NOT cover the lock wait, which is acquired before this deadline's race even opens (see `MAIL_LOCK_WAIT_TIMEOUT_MS`'s row below)* |
| `VERIFICATION_CODE_RESPONSE_FLOOR_MS` | **4000** ⚠️ *raised from 3800 (F4)* | `padToVerificationCodeResponseFloor` | `= PRESEND_ALLOWANCE + LOCK_WAIT + SEND_TIMEOUT` |

### 12.2 Sub-budgets

⚠️ **Corrected (F4, 2026-09-17): `MAIL_LOCK_WAIT_TIMEOUT_MS` is NOT inside `MAIL_SEND_TIMEOUT_MS`** — this section's heading previously said all of §12.2 sat inside the send deadline. `MicroserviceMailTransport.send` acquires the mutex BEFORE opening the `try` that races the rest of the send against `MAIL_SEND_TIMEOUT_MS`, so the lock wait is additive to it, not a sub-budget within it — only `MAIL_PROBE_TIMEOUT_MS` (and the reconnect/publish/confirm remainder) are genuinely inside that race. **Follow-up resolved 2026-09-17:** §12.3 invariant 2 previously summed `LOCK_WAIT + PROBE` against `SEND_TIMEOUT`, the identical containment error one level down — restated below to `PROBE < SEND_TIMEOUT`; `LOCK_WAIT`'s real containment is invariant 1, where F4 already placed it.

| Constant | Value | Inside `MAIL_SEND_TIMEOUT_MS`'s race? | Why it is bounded separately |
|---|---|---|---|
| `MAIL_LOCK_WAIT_TIMEOUT_MS` | **200** | **No** — acquired BEFORE the race opens (additive to `SEND_TIMEOUT`; composed into invariant 1's floor instead, §12.3) | A send that cannot acquire the mutex fails having never touched the broker, and is reported distinctly from a broker failure |
| `MAIL_PROBE_TIMEOUT_MS` | **250** | **Yes** — runs inside `sendLocked`, the promise `raceAgainstDeadline` races | A half-open socket does not throw — it hangs. Without its own bound the probe consumes the whole budget on the exact failure it exists to detect (DD-11) |
| *(remainder ≈ 2750)* | — | **Yes** | Reconnect (TCP + TLS + AMQP) + `checkQueue` + publish + confirm |
...
1. `PRESEND_ALLOWANCE + LOCK_WAIT + SEND_TIMEOUT ≤ FLOOR` — **800 + 200 + 3000 = 4000** ✓ — ⚠️ *completed 2026-09-17 (F4): `LOCK_WAIT` was missing; the floor was `3800` and undercounted the transport's real worst case*
2. `PROBE < SEND_TIMEOUT` — **250 < 3000** ✓ — ⚠️ *restated 2026-09-17 (F4 follow-up): previously written as `LOCK_WAIT + PROBE < SEND_TIMEOUT` (`200 + 250 = 450 < 3000`). `LOCK_WAIT` is not a term of this container at all — `MicroserviceMailTransport.send` acquires the mutex BEFORE opening the `try` that races `sendLocked` (which is where the probe runs) against `SEND_TIMEOUT`, so `LOCK_WAIT` was never actually consumed from this budget. `LOCK_WAIT`'s real containment is invariant 1, above, where F4 already placed it — moving it out of invariant 2 loses no coverage, it removes a term that was never true of this container. **The margin is larger than previously documented, not smaller: `250 < 3000` has more headroom than the old `450 < 3000` claimed — this correction loosens nothing in reality, it stops overstating a constraint that was never binding.***

⚠️ **Invariant 2 is load-bearing for a second reason, recorded during T-4's review.** It is not only a latency budget: it is what keeps the orphaned-send residual narrow. Because the probe is bounded and `PROBE < SEND`, the overall deadline **cannot** fire before `acquireConnection`'s stale-branch teardown has already run — that clock starts only once the lock is already held, so the condition that actually governs it is `PROBE < SEND`, not `LOCK_WAIT + PROBE < SEND` (⚠️ *trigger corrected 2026-09-17, F4 follow-up — same reason as above* ) — so the only window in which an orphaned continuation can overwrite `cached` is a deadline during `connectWithRetry`, whose worst outcome is one leaked connection. **If anyone ever retunes §12 so `PROBE ≥ SEND`, that residual grows from a leak into "an orphan can tear down a live connection out from under a concurrent send."** Any T-9 re-derivation of these values must re-check the residual, not only the latency budget.

Invariant 1 is asserted in `registrations.service.spec.ts` as a pin against the live, composed `VERIFICATION_CODE_RESPONSE_FLOOR_MS` (⚠️ *not a falsification test over the inequality — there is none, and there cannot be one: the floor is DEFINED as the sum, so the inequality cannot fail by construction. The pin's real falsifier is different: change any constituent constant WHILE THE FLOOR STAYS COMPOSED, without updating the pin to match, and it reddens — replacing the sum with a hardcoded literal keeps the pin green too, which is why the composition itself must be held by reading, not by test*); invariant 2 is asserted in `mail-timing.spec.ts`. Falsifying input for invariant 2: raise `PROBE` past `SEND_TIMEOUT` — that test reddens.
...
| **OQ-11** | ⏸️ **STILL DEFERRED — but T-9's numbers now exist, 2026-09-18.** ⚠️ **Measured from Lambda in `eu-west-1`: cold `964 ms`, warm `~209 ms`** (per-kind detail in `tasks.md` T-9 item 5 and `execution.md`). **The Lambda is ~5× faster than the laptop every current bound was derived from** (1132-1366 ms), so `MAIL_SEND_TIMEOUT_MS` — set at ~2.2× the slowest laptop send — now sits far above the worst case observed in region, and the floor it composes is correspondingly generous. **What still blocks the decision is sample size, not access:** the cold figure is **one sample, on one kind**, and locally `contact` (the largest payload) was slowest on all three of its runs, so the kind that matters most for a cold bound is the one never measured cold. The pre-send p99 is also unmeasured. A retune on one cold sample would repeat the original error in the opposite direction — the `1200 ms` bound was reasoned, not measured, and measurement falsified it. **Deferred pending per-kind cold samples.** The original deferral text follows. ⏸️ **DEFERRED to post-T-9 by the product owner (2026-09-16).** ⚠️ **The floor is now 4.0 s per verification request (raised from 3.8 s, F4), and its dominant term is a cold AMQP handshake paid inside the request.** Establishing the connection during **Lambda init** instead would move that cost out of the request path for every invocation after the first, letting `SEND_TIMEOUT` and the floor drop back toward the warm-path cost. It was rejected during design because a broker outage at init would fail the whole function and break every route — but a **non-fatal** init attempt (connect if possible, fall back to lazy) keeps that property. **Deferred, with the reason:** every measurement so far is from a laptop over public internet; the Lambda runs in `eu-west-1` and its latency to this broker is unknown in either direction. Deciding now would mean choosing an architecture from the wrong network. **T-9 produces those numbers, and this decision is taken with them.** Product-owner note (2026-09-16, at the then-current floor), as recorded at the time: 3.8 s on a "send me a code" button is slow but the applicant pays it once, which is what makes deferring affordable. — ⚠️ *restored verbatim (Reviewer, round 6): a prior revision of this line silently rewrote the quote itself from `3.8 s` to `~4 s` to stay consistent with F4's raised floor. `execution.md`'s OQ-11 deferral bullet independently records the figure as **3.8 s** on that date. ⚠️ *That bullet is a Leader-written summary, not a transcript — it corroborates the number and the date, which is the load-bearing point, and must not be cited as her words.* A recorded product-owner statement is evidence, not description — it is never edited to match a changed number; the change is marked BESIDE it instead. This note stands as written, at the then-current `3.8 s` — it is not amended by what follows.* ⚠️ **Product owner, 2026-09-17:** shown F4's raise to `4.0 s` and that the figure derives from laptop measurements T-9 may revise, accepted `4.0 s` **for now** — *"acepto los 4,0 s por ahora"*. A separate, later decision from the `3.8 s` note above, not an amendment to it: today's acceptance is explicitly provisional, pending T-9's real measurements from `eu-west-1`. |
