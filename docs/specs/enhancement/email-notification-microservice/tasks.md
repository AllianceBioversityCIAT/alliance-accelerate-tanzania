...
- [ ] **T-14** Sweep the withdrawn premise across code and tests  (deps: T-9)
      ...
      Verify: `cd backend && npm test --silent` then `grep -rniE '\bSES\b|MessageRejected|sandbox|SendEmailCommand|getSesClient|SesMailTransport|Message\.Body|SesClient|reply-?to' backend/src | grep -v archive`
