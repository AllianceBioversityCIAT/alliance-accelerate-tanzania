...
- [x] **T-9** Observe real delivery on the deployed dev stack  (deps: T-1…T-8)
      ...
      3. ✅ **ESTABLISHED ON THE DEPLOYED STACK (D-F), with one gap named** — CloudWatch, 2026-09-18: `INIT_START` at 10:42:08-09, first send 44 s later at 10:42:53 taking **964 ms** (the cold AMQP handshake, paid inside the request), then four sends at **~209 ms** on the warm container, then a further `INIT_START` at 10:54 after the container went idle. Cold and warm are both observed and both succeeded; **all five sends carry an `attempt`+`outcome` pair**, which is the evidence that no send was frozen mid-flight. ⚠️ **Not observed: a resume — a send on a container that had gone idle and thawed** (that would appear as a send with no preceding `INIT_START`). All five landed inside one 2-minute warm window. D-F's freeze hazard is discharged for the awaited path by construction (every mail dispatch is now awaited — D-I); this observation corroborates it rather than being its sole evidence.
      4. ✅ **ESTABLISHED ON THE DEPLOYED STACK (D-I), 2026-09-18** — the receipt for `REG-2026-0007` logged `attempt` **and** `outcome … status=sent` (209 ms) and **arrived**; the product owner confirmed all five kinds delivered. This is the criterion that found the bug: before the fix the same line pair read `attempt` with **no outcome of either status**. D-G confirmed on the deployed stack in the same pass — HTML correct in all five. Original text: the **receipt** observed as delivered **after** its `201` (⚠️ *corrected 2026-09-17, round 5: this said `202` — that status belongs to `POST /registrations/verify`; `POST /registrations`, which triggers the receipt, returns Nest's implicit default `201` for `@Post()` with no `@HttpCode` — `registrations.controller.ts` confirms both. Pre-existing and spec-wide — `requirements.md`'s D-I row and `design.md` §10's T-9 row carry the same `202`, reported for a separate pass, not swept here*). ⚠️ *Amended 2026-09-17: it was "the one fire-and-forget kind" until D-I was found in production and closed by awaiting the send — so the freeze-drop mechanism this item was written against is gone. **The item is not.** Awaiting proves the broker confirmed the publish, never that the mail arrived; D-H (accepted then dropped at SMTP) has no other evidence anywhere, so this manual observation remains its only evidence. D-G (HTML after `juice`) is different — item 2 above already established it LOCALLY; what this item alone still owes D-G is confirmation on the DEPLOYED stack specifically, where the template resolution or environment prefix could behave differently than in a local script.*
      5. 🟡 **PARTIALLY ESTABLISHED — the headline numbers exist, the sample does not support a retune yet.** Measured from Lambda, `eu-west-1`, 2026-09-18: **cold 964 ms** (`verification-code`, **one sample**), **warm ~209 ms** (`receipt` 209, `rejection` 209, `approval` 208, `contact` 209). ⚠️ **The Lambda is ~5× faster than the laptop** every current bound was derived from (1132-1366 ms) — so `MAIL_SEND_TIMEOUT_MS`, set at ~2.2× the slowest laptop send, is now far above the worst case observed in region. **Still owed, and moved to OQ-11 where the decision lives:** per-kind **cold** samples (only `verification-code` was cold; locally `contact`, the largest payload, was slowest on all three of its runs) and the accepted branch's pre-send p99. Original text: cold **and warm** publish latency measured **from Lambda**, plus the accepted branch's pre-send p99. ⚠️ **Measure per kind, not once:** locally `contact` (the largest payload) was slowest on all three of its runs. These numbers are what **OQ-11** is deferred until — the choice between the current OTP floor (`design.md` §12.1 — not restated here; it changed once already, 2026-09-17, after this line was written) and connecting at Lambda init.
      6. ✅ **ESTABLISHED, 2026-09-18** — zero `overran` lines in the deployed log group over the observation window. The floor was never breached.
      7. ✅ **ESTABLISHED, 2026-09-18** — **73.0 MB** deployed (`CodeSize` on the live function), against NFR-5's 250 MB ceiling. Measured on the artefact that is actually running, which is stronger than measuring a local `sam build` output.
      8. ✅ **ESTABLISHED, 2026-09-18** — every value `MailMicroserviceSecret` supplies resolves into the running function's configuration with a real value, not a placeholder; verified with the scoped `--query` form and a `grep -c REPLACE` that prints a count and never a value. ⚠️ *The criterion said "both dynamic references" — there are three as of the 2026-09-16 `queueName` change. Corrected here.* Original text: the Secrets Manager wiring: both dynamic references resolve into the running function's configuration. Verify with the **scoped** `--query` from `infra/README.md` §7, never a bare `get-function-configuration`.
      ...
      ⛔ **Phase B may not start until this task is `[x]`.** ⚠️ *Historical: Phase B ran ahead of this gate on an explicit product-owner override, recorded as an override in every Phase-B commit. This gate is now satisfied in fact as well as waived in process — the sequence is recorded honestly rather than tidied.*
