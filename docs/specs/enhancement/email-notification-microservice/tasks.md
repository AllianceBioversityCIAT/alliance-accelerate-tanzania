...
- [ ] **T-7** Bound the pre-send window and re-derive the floor  (deps: T-1)
...
- [ ] **T-14** Sweep the withdrawn premise across code and tests  (deps: T-9)
      ...
      Files: `backend/src/registrations/{registrations.service.ts,registrations.service.spec.ts,admin-registrations.service.ts,admin-registrations-reject.spec.ts,admin-registrations.service.spec.ts,registrations-verify.e2e.spec.ts}`, `backend/src/contact/{contact.service.ts,contact.e2e.spec.ts,contact-no-writes.e2e.spec.ts,admin-recipient.resolver.ts}`, `backend/src/mail/{mail.module.ts,no-op-mail.transport.ts}`, `backend/src/test/pii-boundary.spec.ts`, `backend/src/lambda.ts`, `backend/CLAUDE.md`
