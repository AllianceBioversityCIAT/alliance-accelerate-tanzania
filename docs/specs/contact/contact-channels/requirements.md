...
1. **The harness needs a log-capture seam it does not have.** `pii-boundary.spec.ts` is today a response-body scanner containing no log capture whatsoever. The seam must be installed before the request and asserted **after the dispatch promise settles**, because the mail outcome and `.catch()` lines are emitted after the response is written — a spy scoped "around the request" would see only the attempt line.
