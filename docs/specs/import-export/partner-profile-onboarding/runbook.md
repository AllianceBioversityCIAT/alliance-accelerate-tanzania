...
| **Reason breakdown** | Shown only on the **preview** screen (step 4) — a list of reasons rows didn't import, each with a count (e.g., `region: 4` means 4 rows failed because of their region value). This is how you see *why* rows didn't land without reading every row individually. **It does not appear after commit** — read and record it at preview time, before you click "Import N actors." After commit, only the per-row result table and the totals remain — no per-reason counts. |
...
The response shows the same totals (Rows, Created, To skip, Invalid, Warnings) and the same per-row result table as the preview — but **not** the reason breakdown. That only appears on the preview screen (step 4); if you need it, read and record it there, before you commit. The created rows are now real.
...
| **The `Seed Company` sheet's preview shows "To create: 0," the page reads "No rows are eligible to import. Fix the file and upload again," and the "Import 0 actors" button is disabled** | Expected, not a bug — and there is no file to fix. Every row on this sheet quarantines pending an AT-team region pass (DD-11): the sheet has no region column, and its named location column is empty (0 of 11 filled). GPS coordinates are a separate column, filled for 6 of the 11 rows — but GPS alone cannot supply a region (deriving one from coordinates is not allowed), so the quarantine stands regardless. | Nothing to fix — you cannot commit this sheet yet. Record 0 created for this sheet in `reconciliation.md` and move to the next sheet; re-run this sheet once the AT team has supplied a region per organisation. |
| **The total created count doesn't match what you expected for that sheet** | Either more rows quarantined than expected, or fewer. | Compare the reason breakdown you recorded from the preview (step 4) against `mapping.md`'s per-sheet expected counts — the breakdown itself is not shown post-commit, only the per-row result table (step 6); if you didn't record it, that table's per-row failure reasons let you reconstruct the same picture row by row. Some rows are *supposed* to quarantine (that's the whole point of the breakdown). If the mismatch isn't explained by a known quarantine reason, stop and investigate before continuing to the next sheet. |
