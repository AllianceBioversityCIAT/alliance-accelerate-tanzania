...
- [x] **T-8** `mapping.md` — the five remaining sheets  (deps: T-7) — ✅ **PASS on attempt 2 of 3**, both re-audited lenses (conformance, NFR-9/fidelity); the §8 findings lens PASSed on attempt 1. All five sheets dispositioned and closing on their measured denominators (**17 / 10 / 10 / 28 / 53**, 118 columns), traced cell-by-cell over 536 rows. `design.md` **DD-1's open residual is resolved** — all 31 non-ambiguous `Humantarian` locations are exact `CANONICAL_REGIONS` members, so `DISTRICT_TO_REGION` needs no entry and DD-1 closes at 3 contributing sheets. This task's measurement raised **5 findings** against `design.md` §9.1 / `requirements.md` (expected net **757 → 751**), independently verified and **user-approved for amendment as C-6…C-13** — see `execution.md`'s Pivot Record. **No correctness gate exists for this task**; `design.md` §12.1's uncoverable set remains uncovered by design.
...
- [x] **T-10** `reconciliation.md` — skeleton with expected counts  (deps: T-7, T-8) — ✅ **PASS on attempt 2 of 3.** All **1,237** physical source rows accounted for across four buckets, closing on both axes; `imported` = **751** matches `design.md` §9.1's amended net **sheet by sheet**, not merely in total. Attempt 1 FAILed on KZ-001: the 261 QDS `individual` rows — 21% of the workbook — were the only non-`imported`/non-`collapsed` members never enumerated, on a reading that treated two AND-joined FR-8 clauses as alternatives. Fixed by **measuring** the block (rows **28–288**, contiguous — independently confirming §3.1's "sorted by category") rather than deriving it. Raised **F-1** (the 11 `"Retaler"` rows, enumerated and proven disjoint from the contaminated tail — independently confirming **C-5**) and **F-2** (**11 duplicate groups / 24 records** vs `design.md`'s 8/18 — a contradiction always detectable from `mapping.md` §4.5's own prose). §7 and §8 are intentionally blank pending an actual onboarding run. Advisories A-24…A-26 in `execution.md`.
      **Scope:** The reconciliation structure with **expected** figures pre-filled from `design.md` §9.1 (806 candidates → ~751 net, corrected from ~748 to ~752 at the T-7 pivot, to ~757 at the T-7 rework — C-5, and to ~751 at the T-8 pivot — C-8) and actuals left blank for the onboarding run. Per `requirements.md` FR-8's stated reading: per-sheet totals per bucket **plus explicit enumeration by row number** of every `quarantined` and `excluded` row. Includes the cross-sheet duplicate-candidate list (**11 groups / 24 records**, by organisation name and row — corrected from 8 groups / 18 records by T-10's direct measurement, `execution.md` / `reconciliation.md` §6 — **C-14**), the 249 QDS individual exclusions and 3 foreign exclusions **as decisions citing D-1 and D-3**, a slot for the ≥5-row-per-sheet cell-by-cell trace, and a slot for the operator post-commit check result.
...
| | Expected (~751) vs actual, difference explained | T-10 |
