...
## 5. Frontend Design
Routes (`(public)` / `(admin)`), components (map tokens, PII block, forms), data fetching (React Query/SWR), URL-synced filter state, auth guarding (client UX + server enforcement). Reference component inventory and **design tokens** in `docs/ux-ui/design.md §7–8` — no hardcoded colors/spacing.
