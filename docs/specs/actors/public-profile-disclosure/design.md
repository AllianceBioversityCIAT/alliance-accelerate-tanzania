...
### DD-11: A falsifiability claim must state whether its mutation is type-reachable

- **Context.** Three times in this spec a mutation written to prove an assertion discriminates **failed to compile** instead of reddening the assertion: T-9's leak into `toPublicListItem` (`TS2353`), T-13's `actor.phone` on a list-typed consumer, and T-11's conditional spread against a required `PublicActorDetail` property. In each case the explicit-pick projection plus its typed return caught the defect before any test ran. Reaching runtime required a deliberate `as unknown as` bypass — which is precisely what nobody writes by accident.
- **The trap.** "The test caught it" and "the compiler caught it" produce the same green suite and the same confident report, but they license different future actions. A reader who believes a *test* guards a property may delete a type annotation; a reader who believes a *type* guards it may delete the test. T-11's attempt 1 failed on exactly this shape: a docblock claiming a falsification its mutation could not deliver.
- **Decision.** **A mutation-based falsifiability claim must state whether the mutation is type-reachable. If it is not, the guard is the type — say so — and re-aim the claim at the wire-level or value-level property the type does not constrain.**
- **Why the test is still not redundant** (the T-11 Reviewer's sharpening, worth preserving): types describe the *object*; the gate asserts the *wire payload*. `hasOwnProperty` on `res.body` catches a future serialization-layer omission the type cannot see — and the `toBeNull()` half is **not type-guaranteed at all**, since `string | null` happily admits `?? ''` or `?? '—'`. That is a *plausible wrong fix* here, because FR-6 mandates an em-dash placeholder on the frontend. The type owns presence; the test owns nullness. Claiming both for either is the defect.
- *Recorded 2026-09-07 from T-11's review. Generalises KZ-002 to the case where the harness that catches the mutation is the compiler rather than the suite.*
