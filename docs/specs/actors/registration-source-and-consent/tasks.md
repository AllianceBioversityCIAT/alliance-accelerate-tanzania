...
- [x] **T-8** Surface source and consent in the admin actors table  (deps: T-3)
      ...
      **Not done if:** any color or radius is hardcoded instead of using `docs/ux-ui/design.md` §7 tokens (NFR-8), or the new columns' values are dropped or silently truncated on any surface — the stacked cards below `md`, or the horizontally scrolling table at `md`+. *(Corrected 2026-08-04 with the scope correction below: this line previously read "scrolled below `md`", which carried the same false premise — there is no table below `md`.)*
