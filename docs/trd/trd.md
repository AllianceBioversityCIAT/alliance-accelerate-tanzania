# TRD — Technical Requirements Document — ACCELERATE Tanzania Seed Registry

> The technical implementation blueprint. Constitutional baseline. Last reviewed: 2026-08-03.
> Formerly `docs/detailed-design/detailed-design.md` — renamed 2026-08-03. Archived specs still cite the old path and the old section numbers below, which is why **existing section numbers were not renumbered** during that migration.

**Canonical structure map.** AKILI's required TRD structure maps onto this document's stable numbering as follows. Sections 12–13 were added in the 2026-08-03 constitution pass; everything else predates it and keeps its original number so existing `§n` citations stay valid.

| AKILI canonical section | Here |
|---|---|
| 1. System Overview | §1 |
| 2. Architecture Overview & Decisions | **§12** |
| 3. Quality Attribute Scenarios (NFRs) | **§13** |
| 4. Domain Modules & Responsibilities | §2 |
| 5. Data Model & Entities | §3 |
| 6. API Surface & Contracts | §4 |
| 7. Backend Workflows & Business Rules | §5 |
| 8. Frontend Architecture & State Boundaries | §6 |
| 9. Integration Points | §7 |
| 10. Security & Authorization Model | §8 |
| 11. Error Handling & Observability | §9 |
| 12. Testing Strategy | §10 |
| 13. Technical Constraints & Assumptions | §11 |

## 1. System Overview

A decoupled, fully serverless architecture deployed to the **`IBD-DEV`** AWS account/profile.

```
┌──────────────┐     HTTPS      ┌───────────────┐   invoke   ┌──────────────────┐
│  Browser     │ ─────────────► │  CloudFront   │ ─────────► │  S3 (static       │
│ (Next.js SPA │                │  (CDN)        │            │  export of Next)  │
│  static)     │                └───────────────┘            └──────────────────┘
│              │
│              │   /api  HTTPS  ┌───────────────┐  proxy     ┌──────────────────┐
│              │ ─────────────► │  API Gateway  │ ─────────► │ Lambda (NestJS)  │
└──────┬───────┘                └───────────────┘            └────────┬─────────┘
       │ Cognito JWT (Authorization: Bearer)                          │ Prisma
       ▼                                                              ▼
┌──────────────┐                                              ┌──────────────────┐
│ AWS Cognito  │  (user pool, role groups)                    │  RDS MySQL       │
└──────────────┘                                              └──────────────────┘
```

- **Frontend:** Next.js (App Router, TypeScript, Tailwind) → `next build` static export → S3 → CloudFront.
- **Backend:** NestJS (TypeScript) REST API wrapped as a single Lambda handler behind API Gateway, deployed with **AWS SAM** (ADR-008). Prisma client → RDS MySQL.
- **Auth:** Cognito user pool; groups `admin`, `staff`. Unauthenticated callers are `Public`. NestJS guards validate the Cognito JWT and enforce RBAC.
- **AWS profile constraint:** all CLI/IaC/deploy actions use `--profile IBD-DEV`.

## 2. Domain Modules & Responsibilities

| Module (NestJS) | Responsibility |
|---|---|
| `ActorsModule` | CRUD for actor profiles; list/search/pagination; role-aware field projection (PII gating). |
| `CropsModule` | Reference data for the three crops; actor↔crop association. |
| `AuthModule` | Cognito JWT validation, role extraction, guards, decorators (`@Roles`, `@CurrentUser`). |
| `ImportModule` | CSV parsing, validation, transactional bulk upsert, per-row result reporting. |
| `ExportModule` | Role-aware filtered CSV generation (PII enforcement at serialization). |
| `MetricsModule` | Aggregate counts for the landing page (total actors, crops tracked, regions covered). |
| `PrismaModule` | Prisma client provider, connection lifecycle tuned for Lambda. |
| `HealthModule` | Liveness/readiness endpoint. |
| `RegistrationsModule` | Public self-registration intake — versioned consent-policy serving, OTP-gated email verification, and `Registration` storage with **no public read path for any submitted field** (§8). Four public endpoints (§4). Also owns the **admin adjudication surface** (`docs/specs/admin/registration-review-queue/`): five `Admin`-only endpoints — list, detail, approve, reject, dismiss-duplicate (§4) — living in this *same* module by deliberate placement, not a sibling module (that spec's DD-15). **The placement is load-bearing, not cosmetic:** `src/test/pii-boundary.spec.ts`, the PII release gate, derives its checked route set from `MODULE_METADATA.CONTROLLERS` on `RegistrationsModule`; a registrations route registered on a *different* module is invisible to that derivation and would ship uncovered while the suite stayed green. **Nothing enforces this today except the module boundary itself and a controller-level JSDoc — no test asserts that a registrations route added on a *different* module is covered.** Such a route is invisible to both sides of the release gate's totality equality, which derives its expected set from `RegistrationsModule` alone. The narrower case *is* caught: moving `AdminRegistrationsController` out of this module drops five entries from the derived set while the fixture map retains them, so the equality fails by name (`backend/src/test/pii-boundary.spec.ts`). *(Narrowed by validation R9 — the prior wording made an unqualified claim about what tests assert, and a test does assert the controller's membership.)* A future split of `RegistrationsModule` must re-derive the gate's route source, or the constraint recorded here silently lapses. |

Frontend mirrors these as route groups: `(public)` directory/map/profile, `(admin)` management, plus a shared `lib/api` client and `lib/auth` Cognito helper.

## 3. Data Model & Entities

Canonical **Actor** entity derived from the existing field dataset. CSV header → field mapping is authoritative for the import service.

| CSV header | Prisma field | Type | Notes |
|---|---|---|---|
| `Trader_id` | `traderId` | `String @unique` | Natural key from source data; import dedupes on this. |
| `Trader_name` | `traderName` | `String` | Required. Indexed for search. |
| `Region` | `region` | `String` | Indexed (filter + search). |
| `District` | `district` | `String?` | |
| `Trader/processor type` | `traderType` | `String` | Indexed (map filter). Consider enum once values are normalized. |
| `Sex` | `sex` | `String?` | Normalize to `M`/`F`/`Other`/null. |
| `Position` | `position` | `String?` | |
| `Market location` | `marketLocation` | `String?` | |
| `Capacity (volume in t)` | `capacityTons` | `Decimal?` | Numeric; map/filter on ranges. |
| `Technical support required` | `technicalSupport` | `String?` | Free text / multi-select later. |
| `phone` | `phone` | `String?` | **PII** — gated. |
| `Email` | `email` | `String?` | **PII** — gated; validated format. |
| `gpslatitude` | `gpsLatitude` | `Decimal?` | Map plotting. Range −90..90. |
| `gpslongitude` | `gpsLongitude` | `Decimal?` | Map plotting. Range −180..180. |
| `gpsaltitude` | `gpsAltitude` | `Decimal?` | |
| `gpsaccuracy` | `gpsAccuracy` | `Decimal?` | |
| `contact person` | `contactPerson` | `String?` | Named contact at the organisation. Required on the public registration form; optional on import. Publicly disclosed on the DETAIL read for a `GRANTED` actor (`PUBLICLY_DISCLOSED_FIELDS`, §8) — a third party's name, so see ADR-013. |
| `other crops` | `otherCrops` | `String?` `@db.VarChar(300)` | Free-text crops beyond the three tracked value chains. Publicly disclosed on BOTH the list and detail reads. |
| (consent) | `consentStatus` | `ConsentStatus` enum (`GRANTED`/`DENIED`/`UNKNOWN`, default `UNKNOWN`) | Gates public visibility — the public API returns ONLY `GRANTED` actors (`DENIED`/`UNKNOWN` are excluded from every public read and from `/metrics` counts). Enforced in the query, not just the serializer. |
| (derived) | `crops` | `Crop[]` (M:N) | Sorghum / common bean / groundnut. See PRD OQ-1. |
| — | `id` | `String @id @default(cuid())` | Internal PK. |
| — | `createdAt` / `updatedAt` | `DateTime` | Audit timestamps. |

**Canonical template vs. source workbooks.** The table above is authoritative for the **canonical import template** — the CSV headers the import service accepts — not for any particular client-supplied workbook. A source workbook with its own column spellings (e.g. `gpslatitude` variants) or sheet structure is mapped onto this template by a per-source mapping specification before import; it is never read against this table directly. A worked example of that mapping step is produced per onboarding as the `mapping.md` of the relevant import-export spec.

```prisma
// schema.prisma (reference — authoritative shape, finalized during general-setup spec)
enum ConsentStatus {
  GRANTED
  DENIED
  UNKNOWN
}

model Actor {
  id               String   @id @default(cuid())
  traderId         String   @unique
  traderName       String
  region           String
  district         String?
  traderType       String
  sex              String?
  position         String?
  marketLocation   String?
  capacityTons     Decimal? @db.Decimal(10, 2)
  technicalSupport String?  @db.Text
  phone            String?   // PII
  email            String?   // PII
  contactPerson    String?
  otherCrops       String?  @db.VarChar(300)
  gpsLatitude      Decimal? @db.Decimal(10, 7)
  gpsLongitude     Decimal? @db.Decimal(10, 7)
  gpsAltitude      Decimal? @db.Decimal(10, 2)
  gpsAccuracy      Decimal? @db.Decimal(10, 2)
  consentStatus    ConsentStatus @default(UNKNOWN)  // public API returns GRANTED only
  crops            CropsOnActors[]
  createdAt        DateTime @default(now())
  updatedAt        DateTime @updatedAt

  @@index([region])
  @@index([traderType])
  @@index([consentStatus])
  @@index([traderName])
}

model Crop {
  id     String @id @default(cuid())
  name   String @unique   // "sorghum" | "common_bean" | "groundnut"
  actors CropsOnActors[]
}

model CropsOnActors {
  actor   Actor  @relation(fields: [actorId], references: [id], onDelete: Cascade)
  actorId String
  crop    Crop   @relation(fields: [cropId], references: [id], onDelete: Cascade)
  cropId  String
  @@id([actorId, cropId])
}
```

**Disclosure set (single source of truth, revised by `actors/public-profile-disclosure`):** consent (`consentStatus = GRANTED`) gates disclosure of every field an actor supplied, not field identity. Four constants in `src/common/pii-consent.policy.ts` carry the policy: `PUBLICLY_DISCLOSED_FIELDS` (`phone`, `email`, `sex`, `position`, `marketLocation`, `contactPerson`, `otherCrops`) — a *presence* set, required on the single-actor detail read (`GET /api/v1/actors/:id`) for a `GRANTED` actor; `CONTACT_BLOCK_FIELDS` (`phone`, `email`, `position`, `marketLocation`, `contactPerson`) — its subset, an *absence* set on the list read (`GET /api/v1/actors`), by key and by value, under every filter/page/page-size; `NEVER_PUBLIC_FIELDS` — an absence set on **every** public path regardless of consent, now including `technicalSupport` (unreviewed staff-authored free text, not an actor PII declaration) alongside `traderId`, `gpsAltitude`/`gpsAccuracy`, and the registration-provenance columns; and `PII_ALLOWLIST` — retained, **empty**, as the one-file re-restriction point should legal narrow disclosure again. Any new PII-adjacent field is classified in this module (and only there); the role-aware serializer's two projections (`toPublicListItem`, `toPublicDetail`) build public output by explicit allowlist, so the implemented set is exactly what this module declares. Exact GPS (`gpsLatitude`/`gpsLongitude`) is additionally **consent-gated**: it is surfaced only for `GRANTED` actors and withheld (`gps: null`) for non-`GRANTED`, on both list and detail.

### 3.1 Registration & EmailVerification (public self-registration)

Two entities and three supporting counter tables behind the public self-registration intake, all distinct from `Actor` and outside the guarantees above. **`PII_ALLOWLIST` and `NEVER_PUBLIC_FIELDS` are `Actor` column lists** consumed by the `Actor` serializer — they do not, and structurally cannot, cover a table that is not `Actor`. `Registration`'s PII is instead contained **structurally**: its public surface returns at most two or three scalars and never reads the `payload` column (§8).

| Model | Purpose | PII |
|---|---|---|
| `RegistrationStatus` (enum) | `PENDING_REVIEW · AWAITING_APPLICANT · APPROVED · REJECTED · WITHDRAWN`. `AWAITING_APPLICANT` and `WITHDRAWN` are declared now but unreachable until chunk 4, so that chunk needs no enum migration; `APPROVED` and `REJECTED` become reachable with chunk 3b's adjudication (`docs/specs/admin/registration-review-queue/`). | — |
| `Registration` | One row per submission: `reference` (the applicant-facing key), `status`, `payload` (the submission, JSON, admin-only in its entirety), `submitterEmail` (the OTP-verified address — the one later published as `Actor.email` on approval), `emailVerifiedAt`, `consentAcceptedAt` (the server-witnessed instant the submission arrived — an **upper bound** on the applicant's true acceptance moment, not a client-attested one), `consentPolicyVersion`. Also declares the adjudication columns (`publishedActorId`, `reviewedBySub`/`reviewedByEmail`/`reviewedAt`, `rejectionReason`, `reviewNote`, `duplicateDismissals`) that only `docs/specs/admin/registration-review-queue/` writes, so that spec needs no migration of its own. | `payload`, `submitterEmail` |
| `EmailVerification` | The pre-verification store: `email`, an HMAC-SHA-256-hashed OTP code (plaintext never stored, never logged), an attempt counter, expiry, single-use marker. Not a `Registration` — no row here is ever readable by any caller. | `email` |

Three further tables exist as concurrency-safe counters behind the abuse-resistance posture in **§12.5, ADR-010**: `EmailSendBudget` (the atomic per-email OTP send cap), `RegistrationSequence` (the race-safe reference allocator), and `RegistrationLookupAttempt` (the persistent, per-caller-and-reference bound behind the public status lookup — its `ip` column is personal data under GDPR Art. 4(1) / CJEU *Breyer*).

**Four tables now hold personal data with no retention or deletion policy** — `Registration.submitterEmail`, `EmailVerification.email`, `EmailSendBudget.email`, `RegistrationLookupAttempt.ip`. This is a recorded, accepted risk (PRD OQ-4), not an oversight: stating it here keeps the public surface honestly sized rather than implied smaller than it is.

## 4. API Surface & Contracts

REST, JSON, versioned under `/api/v1`. List endpoints are paginated (`?page`, `?pageSize`) and filterable. Field projection is role-aware for `staff`/`admin` (full record, every path) and, for `Public`, consent-aware on the detail path only: a `GRANTED` actor's contact block is included in `GET /api/v1/actors/:id` and nowhere else on a public path (§8).

| Method & path | Auth | Description |
|---|---|---|
| `GET /api/v1/metrics` | Public | Landing-page aggregates. |
| `GET /api/v1/actors` | Public | Paginated/filterable list (`q`, `region`, `crop`, `traderType`, `capacityMin/Max`). Returns the **list set** only — `CONTACT_BLOCK_FIELDS` (`phone`, `email`, `contactPerson`, `position`, `marketLocation`) never appear here, for any actor, under any filter or page size — this is the bulk-exposure boundary the map/dashboard/CSV inherit. |
| `GET /api/v1/actors/:id` | Public | Single actor. For a `GRANTED` actor, returns the full **published set** including the contact block — the only public path that does. Non-`GRANTED` → `404`, indistinguishable from a missing id. |
| `POST /api/v1/actors` | Staff/Admin | Create (validated DTO). |
| `PATCH /api/v1/actors/:id` | Staff/Admin | Update (validated DTO). |
| `DELETE /api/v1/actors/:id` | Admin | Delete. |
| `POST /api/v1/import` | Admin | Multipart CSV upload → per-row result report. |
| `GET /api/v1/registrations/consent-policy` | Public | Versioned consent-policy text — one source for the API and the form UI. |
| `POST /api/v1/registrations/verify` | Public | Sends an OTP to the supplied email. **Always `202`, empty body** — deliverable, undeliverable, already-known, and over-cap addresses all get the identical response, so the endpoint cannot be used to test whether an address exists. |
| `POST /api/v1/registrations` | Public | Creates a `Registration` once the OTP is verified. Returns **only** `{ reference }` — no payload echo, no internal `id`. |
| `POST /api/v1/registrations/lookup` | Public | Status by `{ reference, email }` in the request **body**, never a query string (keeps an email address out of request lines, `Referer`, and history). `404` is byte-identical for an absent reference, an email mismatch, and the endpoint's own rate-limit lockout — none is distinguishable from another. |
| `GET /api/v1/admin/registrations` | Admin | Paginated/filterable adjudication queue (`status`, `q`, `region`, `traderType`, `sort`, `page`, `pageSize`); each row carries a `duplicateCandidateCount`, never the candidates themselves. |
| `GET /api/v1/admin/registrations/:id` | Admin | Full registration detail: payload, consent record, duplicate candidates, activity trail. `404` here is a plain not-found, distinguishable from `403` — the public lookup's uniformity requirement (above) stops at the auth boundary; an authenticated Admin is entitled to know whether a registration exists. |
| `POST /api/v1/admin/registrations/:id/approve` | Admin | Single-transaction, compare-and-set approval (`id` + `status = PENDING_REVIEW`; zero rows ⇒ `409` when the row exists, `404` when it does not, DD-22): creates the `Actor`, derives `traderId` (`SR-<year>-<seq>`), writes the literal-picked publishable subset, audits inside the same transaction, links `publishedActorId`. A pre-existing `traderId` also yields `409`, naming the colliding key — never a `500` (ADR-012). |
| `POST /api/v1/admin/registrations/:id/reject` | Admin | Structured reason from a frozen list + optional applicant-facing note; same compare-and-set refusal as approve; creates no `Actor`; the stored consent record is untouched. |
| `POST /api/v1/admin/registrations/:id/dismiss-duplicate` | Admin | Per-candidate dismissal by candidate id (never an index, since detection re-running can reorder or resize the candidate set); persisted on `Registration.duplicateDismissals`. |
| `GET /api/v1/users`, role mgmt | Admin | Cognito-backed user/role administration. `POST /api/v1/users` (create; `admin/user-management` FR-3 originally specified a response that **MUST NOT include any password** — superseded after deployment by `feature/admin-credential-handoff`, PR #45, which has no spec folder of its own and is recorded only in `bugfix/admin-user-invite-and-reset` archive-summary.md §10) returns `201 { user, temporaryPassword, emailSent }`; `POST /api/v1/users/:id/password` (reset; `admin/user-management` FR-7 likewise specified that *"the admin never sees or sets a plaintext password through this endpoint"* — superseded by the same PR #45) returns `200 { temporaryPassword, emailSent }`. Both dispatch an application-level email via `MailService` after the Cognito call (`auth/account-access-emails` T-4/T-5) — Cognito itself sends nothing for either call (`MessageAction: 'SUPPRESS'` on create; no `MessageAction` field exists on the reset command). `emailSent` reports only that the dispatch was accepted without throwing, never a delivery receipt, and is `true` under `MAIL_TRANSPORT=no-op`; `temporaryPassword` is always populated regardless of `emailSent` (§12.1's 2026-09-21 correction). |
| `POST /api/v1/contact` | Public | Stateless relay to the current Cognito `admin` group (or `CONTACT_FALLBACK_RECIPIENT`), never persisted. `202` empty body on acceptance; `400` field errors, `413` oversize, `429` throttled, `502` transport rejection, `500` unresolvable recipient. |
| `GET /api/v1/health` | Public | Health check. |

The four public `registrations*` paths are the API's first unauthenticated **write** path (`POST /registrations`) and its first unauthenticated read of anything beyond public directory data (`POST /registrations/lookup`, gated to two scalars). Field projection for them is not role-aware filtering — see §8's structural-containment note. The five `admin/registrations/*` paths are the module's first `Admin`-only, PII-dense surface over that same table — two reads (the queue list, the detail view) and three writes (approve, reject, dismiss-duplicate) — and `approve` is the **only** route in the system that turns a `Registration` into a public `Actor` (§8, ADR-012).

**Three rows were removed here on 2026-09-21 — deliberately, and they must not come back (ATP-53, ATP-68).**

- `GET /api/v1/export` (Staff/Admin filtered CSV stream) — **cancelled, not deferred.** A server-side, role-aware admin export is out of MVP scope (PRD §5 *Out of Scope*); ATP-53 is closed as cancelled. The `/dashboard` **Download view** button is a *different* artefact and stays: public audience, built in the browser from the list projection (`frontend/lib/dashboard/csv.ts`), and therefore structurally incapable of carrying an actor's contact block. It does not replace the admin export and must not be re-documented as if it did.
- `GET /api/v1/actors/geo` (narrow points feed for the map) — **not built, by decision, and the decision is measured.** The map loads full list-projection records through the ordinary paginated `GET /api/v1/actors` and always did; this row described a tactic the project abandoned when `bugfix--map-loads-all-actors` shipped (PR #20, `28d4c84`) and never re-documented. Measured 2026-09-21 before deciding: at 1 000 actors the narrow projection would have sent 30 KB gzipped against the list projection's 37 KB. **7 KB.** Response compression (`common/compression.config.ts`) and a larger page size were the changes that actually moved the number; a second endpoint was not. See QA-6 for the tactic as it now stands.
- `GET /api/v1/crops` (public crop reference list) — **not built, by decision.** The three MVP crops live in a frontend constant (`frontend/lib/content/crops.ts`); adding one is a code change plus a deploy, and that cost is accepted. Build the endpoint only if crops must become editable without a deploy.

**QA-1 (§13) was trimmed to match.** Its public-read-path list named `/export` and `/actors/geo`; both are gone, because neither route exists and neither is going to. A quality scenario naming an absent route verifies nothing — worse, it passes: the request falls through to `@Get(':id')`, `404`s, and the PII assertion then runs against an error body.

Both rows documented routes **no controller has ever implemented**, and that is not a harmless inaccuracy: this table is read by agents. A request to a documented-but-absent `/api/v1/...` path under `actors` falls through to `@Get(':id')`, `404`s, and a PII assertion against that `404` body then passes forever while exercising nothing — KZ-002's *"a gate that cannot run cannot fail"* in its passing-vacuously form. Add a row here only when a controller answers it.

**Conventions:** DTO validation via `class-validator`; consistent error envelope `{ statusCode, message, error, details? }`; pagination envelope `{ data, page, pageSize, total }`. See skills `api-design-principles`, `error-handling-patterns`.

## 5. Backend Workflows & Business Rules

- **Role-aware / consent-aware projection:** a single serialization layer enforces PII visibility server-side on every read path (list, detail). For `staff`/`admin` the full record is included on every path; for `Public`, the contact block is withheld everywhere except the single-actor detail read, where it is included only for a `GRANTED` actor (§8). Never relied upon in the client.
- **CSV import:** parse (streaming) → validate each row against the Actor DTO → upsert by `traderId` inside a transaction (or batched transactions) → return `{ inserted, updated, failed: [{ row, errors }] }`. A bad row never corrupts committed rows.
- **CSV export:** apply the same filters as the directory; serialize through the role-aware projector — a Public-scope export carries consent-disclosed fields (`sex`, `otherCrops` — no longer blanket PII under `actors/public-profile-disclosure`) but can never leak the contact block (`contactPerson`, `position`, `phone`, `email`, `marketLocation`) or `NEVER_PUBLIC_FIELDS`.
- **GPS validation:** latitude ∈ [−90,90], longitude ∈ [−180,180]; rows outside range import with GPS nulled + flagged, not plotted.
- **Metrics:** computed via aggregate queries (count distinct regions, count actors with GPS, crops tracked); cacheable.

## 6. Frontend Architecture & State Boundaries

- **Next.js App Router**, **static export** (`output: 'export'`) — no SSR/Next API routes (incompatible with pure S3/CloudFront static hosting). All dynamic data is fetched client-side from the NestJS API.
- **Data fetching:** typed `lib/api` client (fetch wrapper) + React Query (or SWR) for caching/pagination/filter state. Server state stays in the query cache; UI state (filters, modals) in local/URL state. Filters are URL-synced so views are shareable.
- **Auth:** Cognito (Amplify Auth or `oidc-client`) stores JWT; an auth context exposes role; admin routes are client-guarded **and** enforced server-side (client guard is UX only).
- **Map:** Leaflet via a dynamically imported client component (`ssr: false`), fed by the ordinary paginated `GET /api/v1/actors` — `useDashboardActors` accumulates every matching actor across sequential pages (`DASH_PAGE_SIZE` 500 × `DASH_MAX_PAGES` 10 = a 5 000-actor ceiling), because a map cannot paginate: "page 2 of the geography" is not a thing a visitor can use. Crop-colored markers; filter panel drives query params. When the registry exceeds the ceiling the rail states the shortfall rather than captioning a partial map with the full count (QA-6).
- **Styling:** Tailwind + tokens from `docs/ux-ui/design.md`; shadcn/ui primitives. No hardcoded colors/spacing outside tokens.

## 7. Integration Points

- **AWS Cognito** — identity, JWT issuance, role groups.
- **AWS RDS MySQL** — primary datastore (Prisma). ⚠️ *Corrected 2026-09-17:* the Lambda is **not** VPC-attached — `infra/20-backend/template.yaml:203`, `# NO VpcConfig - Lambda runs outside the VPC (DD-2)` — and reaches RDS over its public endpoint. Use a connection strategy safe for Lambda concurrency (RDS Proxy or constrained pool) regardless.
- **API Gateway + Lambda** — single NestJS handler via **`serverless-http`** (API Gateway HTTP API v2; see `backend/CLAUDE.md`'s serverless-http lesson on the body-parsing gotcha — the choice of adapter is load-bearing, not interchangeable), deployed with **AWS SAM** (ADR-008: SAM is the only IaC tool in this project; `infra/` holds the SAM templates, not a Serverless Framework `serverless.yml`).
- **OneCGIAR Notification Microservice** — email leaves the system through this external service, reached over a RabbitMQ broker under a publisher confirm (`MicroserviceMailTransport`), never Amazon SES (removed; see ADR-015, `enhancement/email-notification-microservice`).
- **S3 + CloudFront** — static frontend hosting/CDN.
- **Google Analytics 4** (`gtag.js`, loaded from `googletagmanager.com`) — the project's **first third-party client-side integration** and its first client-side transfer of visitor data to a third party. Default measurement only: no custom events, parameters, or dimensions. Injected by the browser only after explicit consent, mounted in the `(public)` layout; **no server component participates** — the API never sees an analytics call, and the measurement ID is a build-time `NEXT_PUBLIC_*` value, not a secret (see §8 for what belongs in SSM).
- **All provisioning/deploy** runs under `--profile IBD-DEV`.

## 8. Security & Authorization Model

- **Roles:** `Public` (anonymous, read-only; the contact block appears only on the single-actor detail read, and only for a `GRANTED` actor's own consented fields — never on list, geo, dashboard, or export), `Staff` (read incl. PII, create/edit), `Admin` (full incl. delete, import, user mgmt).
- **Enforcement:** NestJS `JwtAuthGuard` (validates Cognito JWT signature/claims) + `RolesGuard` (`@Roles('admin')` etc.). PII projection enforced in the serialization layer independent of route guards (defense in depth).
- **Disclosure set (implemented):** `PII_ALLOWLIST` in `src/common/pii-consent.policy.ts` is retained, **empty** — legal has narrowed nothing (yet); it is the one-file re-restriction point if it later does. The boundary is carried instead by two other constants in the same module: `PUBLICLY_DISCLOSED_FIELDS` (`{ phone, email, sex, position, marketLocation, contactPerson, otherCrops }`) — required present, by value, on the single-actor detail read (`GET /api/v1/actors/:id`) for a `GRANTED` actor — and its subset `CONTACT_BLOCK_FIELDS` (`{ phone, email, position, marketLocation, contactPerson }`) — required absent, by key and by value, from the list read (`GET /api/v1/actors`), under every filter/page/page size, so the map/dashboard/CSV structurally cannot carry it. `technicalSupport` sits in `NEVER_PUBLIC_FIELDS` instead — unreviewed staff-authored free text, not an actor PII declaration — alongside `traderId`, `gpsAltitude`, and `gpsAccuracy`, all likewise never public on any path. **Exact GPS is consent-gated:** `gpsLatitude`/`gpsLongitude` are surfaced only for `GRANTED` actors (withheld as `gps: null` for `UNKNOWN`/`DENIED`), on both list and detail, and non-`GRANTED` actors are excluded from every public read and from `/metrics` counts. Consent is pinned in the Prisma `WHERE` (not serializer-only) and the boundary — including the list/detail asymmetry — is proven end-to-end over HTTP in `src/test/pii-boundary.spec.ts` (NFR-1, NFR-7).
- **Unapproved-PII boundary (public self-registration).** `Registration.payload` and `Registration.submitterEmail` (§3.1) are PII for an organisation that has not been approved for publication and may never be. **`PII_ALLOWLIST` and `NEVER_PUBLIC_FIELDS` cannot protect this table** — both enumerate `Actor` columns for the `Actor` serializer, and neither one sees a `Registration` row. Protection here is **structural, not filtered**: the public surface (`GET /registrations/consent-policy`, `POST /registrations/verify`, `POST /registrations`, `POST /registrations/lookup`) returns at most two or three scalars (`reference`, `status`, `reviewNote`) and never reads the `payload` column or the internal `id`, before or after adjudication. Proven end-to-end over HTTP by extending `src/test/pii-boundary.spec.ts` to this module's paths — release gate, alongside QA-1's existing coverage (see QA-12, §13).
- **Adjudication authority (public self-registration → public `Actor`).** The five `admin/registrations/*` endpoints (§4) are the **only** path by which a `Registration` becomes a public `Actor`. All five are `Admin`-only: `Staff` → `403`, anonymous → `401` (QA-3). The approve/reject transactions close the double-adjudication race by construction, not by a read-then-check: a conditional update matching `id` **and** `status = PENDING_REVIEW` is the first write, and zero affected rows *is* the refusal (`409`), so there is no window in which a second request can race the first. The reviewer's identity (`reviewedBySub`/`reviewedByEmail`) is resolved server-side from the validated JWT and `ActingAdminResolver` — never client-supplied. The created `Actor` is the only public artifact an approval ever produces; `Registration.payload` and `submitterEmail` stay structurally contained (ADR-010) **before and after** adjudication (ADR-012) — approval publishes a projected subset of the payload, never the row itself.

**Transport:** HTTPS everywhere (CloudFront + API Gateway). CORS locked to the CloudFront origin.
- **Secrets:** DB credentials and Cognito config from AWS SSM Parameter Store / Secrets Manager — never committed.
- **Input safety:** all writes go through validated DTOs; Prisma parameterizes queries (no raw SQL on user input).

## 9. Error Handling & Observability

- Global NestJS exception filter → consistent error envelope; no stack traces leaked to clients.
- Validation errors return `400` with field-level `details`.
- Structured JSON logging to CloudWatch (request id, route, role, latency); import jobs log summary counts.
- Lambda cold-start mitigation: lazy Prisma client reuse across invocations, minimal bundle. See skill `aws-serverless`.

## 10. Testing Strategy

- **Backend unit/integration:** Jest + Supertest per module; contract tests asserting AC-1's list/detail split — the contact block is omitted for `Public` on every read path except the single-actor detail read, where it is present only for a `GRANTED` actor; import service tests for partial-failure isolation (AC-5).
- **Frontend:** component tests (Testing Library) for the profile's consent-gated Contact section (`ProfileContact`), filters, forms; the directory list as the accessible equivalent of the map.
- **E2E (smoke):** core flows — public browse, login, create actor, import, export-respects-role.
- Verification commands wired per package (`npm run test`, `npm run build`; lint is `npx eslint "{src,test}/**/*.ts" --quiet` in `backend/` — never `npm run lint` there, it is `eslint --fix` and mutates the diff under review — and `npm run lint` in `frontend/`; root `CLAUDE.md` § Verification commands).

## 11. Technical Constraints & Assumptions

- **Mandated stack:** Next.js static export + S3/CloudFront; NestJS on Lambda + API Gateway; RDS MySQL; **Prisma**; **Leaflet**; **Cognito**.
- **Mandated AWS profile:** `IBD-DEV` on every AWS command, script, and IaC definition (PRD AC-7).
- **Static-export constraint:** no Next SSR/ISR/route handlers — keep all server logic in NestJS.
- **Lambda DB constraint:** manage MySQL connections for serverless concurrency (RDS Proxy recommended).
- **Scale target:** 1,000+ actors; design list/map/queries to remain performant (indexes, paginated/geo endpoints, optional clustering).

---

## 12. Architecture Overview & Decisions

*(AKILI canonical section 2. Added 2026-08-03 — documents decisions the system already embodies, so it is a record of the as-built architecture, not a redesign.)*

### 12.1 C4 — Level 1: System Context

```
        ┌───────────────────────┐            ┌────────────────────────┐
        │ Public visitor        │            │ Staff / Admin          │
        │ (anonymous, Public)   │            │ (Cognito user)         │
        └───────────┬───────────┘            └───────────┬────────────┘
                    │ browses map, directory & profiles  │ manages actors,
                    │ (contact block only if consented)  │ imports, exports, users
                    ▼                                    ▼
        ┌───────────────────────────────────────────────────────────────┐
        │        ACCELERATE Tanzania Seed Registry  [this system]       │
        │  Public seed-system map, actor directory, admin back-office   │
        └───────┬───────────────────────────────┬───────────────────────┘
                │ authenticates; generates,     │ publishes verification-code,
                │ expires & verifies reset      │ contact, registration receipt
                │ codes                         │ & outcome, and reset mail
                ▼                               ▼
        ┌───────────────────┐           ┌─────────────────────────────────┐
        │ AWS Cognito       │           │ OneCGIAR Notification           │
        │ [external]        │           │ Microservice [external]         │
        └───────────────────┘           └─────────────────────────────────┘
```

**Legend.** Boxes are people or systems. `[this system]` is what we build; `[external]` is a system we depend on but do not implement — AWS-managed (Cognito) or run by another CGIAR platform team (the notification microservice). Arrows point in the direction of the call and are labelled with intent.

⚠️ **Corrected 2026-09-17 (`enhancement/email-notification-microservice`, FR-6).** This box previously read `AWS SES [external]` with the arrow labelled *"sends invites / resets"* — false even before this change: `UsersService.create` calls `AdminCreateUser` with `MessageAction: 'SUPPRESS'` (no invite email is ever sent; the admin shares the temporary password out-of-band), and `UsersService.resetPassword` calls `AdminSetUserPassword({ Permanent: false })`, which also sends no mail. Neither admin-mediated flow was ever an SES call. The one Cognito-sent email that does exist — the user-initiated `/forgot-password` flow (a staff/admin Cognito user; there is no anonymous-visitor account to reset) — goes through Cognito's own `COGNITO_DEFAULT` mailer (`infra/10-data-auth/template.yaml`), unconditionally since this spec's Phase B removed the `EnableSesSending`/`UseSes` conditional. The box that actually sends mail today is the OneCGIAR Notification Microservice: `MicroserviceMailTransport` publishes onto a RabbitMQ queue under a publisher confirm; the microservice consumes it, validates the API key against CLARISA, and relays over SMTP (`design.md` §1, ADR-015).

⚠️ **Corrected again 2026-09-21 (`auth/account-access-emails`, T-4/T-5/T-6).** The paragraph above's claims that "no invite email is ever sent" and `AdminSetUserPassword` "sends no mail" are now false — **but only at the application layer, not at the Cognito layer**, and that split matters: `AdminCreateUserCommand` still runs with `MessageAction: 'SUPPRESS'` and `AdminSetUserPasswordCommand` still carries no `MessageAction` field, unchanged since the correction above — Cognito itself still sends nothing for either call, and no SES/Cognito mail arrow was added to this diagram. What changed is that `UsersService.create` and `UsersService.resetPassword` now each make an additional, application-level call to `MailService` (this system's existing OneCGIAR Notification Microservice channel, same box as above — no new external dependency) immediately after the Cognito call succeeds: `create()` dispatches an invitation mail (T-4), `resetPassword()` dispatches an admin-reset mail (T-5). Both endpoints' response bodies now carry `emailSent: boolean` alongside the always-populated `temporaryPassword` (§4 above) — `emailSent` reports only that the microservice dispatch was accepted without throwing, never that a mailbox received anything; it is `true` under `MAIL_TRANSPORT=no-op`. The admin-mediated out-of-band password handoff (FR-2/FR-5) is unchanged and remains the guaranteed fallback regardless of `emailSent`.

⚠️ **Corrected again 2026-09-25 (`auth/forgot-password-delivery`, T-7).** The first paragraph above's claim that the user-initiated `/forgot-password` flow "goes through Cognito's own `COGNITO_DEFAULT` mailer... unconditionally" is now false, and — separately — it was already false when written. **What is true now:** a `CustomEmailSender` Lambda trigger (`infra/10-data-auth/functions/custom-email-sender/`) supersedes `EmailConfiguration` entirely. Once the trigger is set, Cognito hands the encrypted code to this function instead of sending mail itself; the function decrypts it (a customer-managed symmetric KMS key; `kms:Decrypt` is granted to the function's own execution role and scoped to that one key, never `*`) and publishes it over AMQP to the same OneCGIAR Notification Microservice channel named above — no new external dependency. Cognito still authenticates and still owns the reset-code lifecycle — it issues the code and verifies it against the value the user submits (FR-3) — only delivery moved. **Verified live on `accelerate-tz-dev`, 2026-09-25, by one real password reset** (not inferred from the template): the mail arrived in ~10 s; exactly one code was received, so Cognito did not retry; the function's own log read `dispatched triggerSource=CustomEmailSender_ForgotPassword` and completed in 1953.61 ms plus 598.71 ms init — ≈2.55 s of cold-path latency against Cognito's own non-configurable trigger ceiling, a limit of unknown size that this spec's `design.md` DD-3a is the canonical home for, not against this function's own 15 s Lambda timeout; the log carried neither the recipient address nor the code (NFR-1); and a live-pool diff showed the 3 predicted keys changed (`LambdaConfig`, plus §6's two (c)-class resets) plus `LastModifiedDate`, which changes mechanically on any update and §6 bucketed as not applicable; 20 keys untouched. That is one reset by one user on one environment — enough to close `requirements.md` §8's three gaps with no automated gate (D-3 real decryption, D-4 the KMS grant chain, D-6 delivery), not a load test. **Why the 2026-09-17 text read as true and was not:** its source was a *template* edit — **`enhancement/email-notification-microservice`'s** Phase B removing the `EnableSesSending`/`UseSes` conditional — that had never deployed, because `DEPLOY_INFRA` defaults to `false`. The live pool was still `DEVELOPER` plus an SES identity when that paragraph was written, a premise `requirements.md` §2 records as falsified before this spec began and `T-5`'s `pool-before.json` captured as fact on 2026-09-23. The paragraph described the template while asserting it of the deployed system — the pattern the kaizen log tracks as KZ-008, here caught on the same claim it was made about.

### 12.2 C4 — Level 2: Containers

```
┌─────────────────────────────────────────────────────────────────────────────────────────────┐
│                             ACCELERATE Tanzania Seed Registry                               │
│                                                                                             │
│  ┌────────────────────────┐ HTTPS /api/v1   ┌─────────────────────────┐                     │
│  │ Web SPA                │ ───────────────►│ REST API                │                     │
│  │ [Next.js static export]│ Bearer JWT      │ [NestJS on AWS Lambda]  │                     │
│  │ S3 + CloudFront        │                 │ behind API Gateway HTTP │                     │
│  │ Leaflet map, directory │◄─────────────── │ guards + role-aware     │                     │
│  │ admin back-office      │ JSON envelopes  │ serializer (PII gate)   │                     │
│  └────┬───────────────────┘                 └───┬─────────────────────┘ ──┐─────────────┐   │
│       │                                         ▼ Prisma                  │             │   │
│       │                                     ┌───┬─────────────────────┐   │             │   │
│       │                                     │ Relational DB           │   │             │   │
│       │                                     │ [AWS RDS MySQL 8]       │   │             │   │
│       │                                     │ Actor, Crop, consent    │   │             │   │
│       │                                     └─────────────────────────┘   │             │   │
└───────┼───────────────────────────────────────────────────────────────────┼─────────────┼───┘
        │ OIDC / JWT                                                        │ IAM + SG    │ AMQP publish
        ▼                                                                   ▼             ▼
┌───────────────────┐                                           ┌───────────────────┐   ┌─────────────────────────────────┐
│ AWS Cognito       │                                           │ Secrets Manager   │   │ OneCGIAR Notification           │
│ [external]        │                                           │ [external]        │   │ Microservice [external]         │
└───────────────────┘                                           └───────────────────┘   │ RabbitMQ queue, publisher       │
                                                                                        │ confirm; CLARISA checks the     │
                                                                                        │ API key, then SMTP relay.       │
                                                                                        │ Posts EVERY subject to Slack    │
                                                                                        │ — the only D-H signal that      │
                                                                                        │ exists (design.md §6, R-4)      │
                                                                                        └─────────────────────────────────┘
```

**Legend.** Each box is a separately deployable/runnable unit; `[…]` names its technology. The Web SPA holds **no** server logic — it is a static artifact, so every dynamic read crosses the HTTPS boundary to the REST API. `Secrets Manager` now also resolves the notification microservice's configuration and credentials (`MailMicroserviceSecret` — see `design.md` §4.5 and `infra/20-backend/template.yaml` for the values it supplies, DD-6), alongside the pre-existing `DB_PASSWORD`/`OTP_HMAC_SECRET`. Deployment topology for these containers is `docs/infrastructure.md` §2.

⚠️ **The notification microservice is new to this diagram, not swapped in for a prior box** — no external mail system was ever shown at Container level before this change (only at Context level, §12.1, as the now-corrected `AWS SES` box). REST API reaches it via `MicroserviceMailTransport`: `amqplib`, a publisher-confirm channel, `checkQueue` (never a topology write — `design.md` DD-3), under one bounded send deadline (`MAIL_SEND_TIMEOUT_MS`, §12.1 of `design.md`). Everything past the queue — the microservice's own CLARISA check and SMTP relay — is that external system's internals and is collapsed into one box here, exactly as Cognito's internals are. The one thing that box does **not** collapse away: it posts **every** message's subject line to Slack (`#microservices-notifications`), not only failing ones (`design.md` §6, `proposal.md` R-4) — the box above says so rather than implying a failure-only post (ADR-015).

### 12.3 Architecture style

**Modular monolith, deployed serverless.** One NestJS application, internally partitioned into the domain modules of §2, packaged as a *single* Lambda handler. Not microservices: the modules share a datastore and a transaction boundary (the CSV import's per-row isolation depends on it), and the actor count — 1,000s, not millions — does not justify independent scaling units. Frontend and backend are decoupled at the network boundary only.

**Tactics chosen** (against the scenarios in §13): role-aware **serialization allowlist** + consent pinned in the Prisma `WHERE` (information-hiding, defense in depth); **paginated reads + response compression** (gzip above 1 KB in both bootstraps, `common/compression.config.ts`; a 500-actor page is 140 KB plain and 18 KB gzipped — measured, and the reason the once-planned `/actors/geo` narrow projection was dropped rather than built); **indexed filter columns** (`region`, `traderType`, `consentStatus`, `traderName`); **connection reuse across Lambda invocations** (cold-start and connection-exhaustion mitigation); **stateless compute** (availability via managed redundancy rather than custom failover).

### 12.4 Robust-vs-lite tier decision

**Tier: LITE.** Recorded as **ADR-001** below and cited by `docs/infrastructure.md`.

| Robust-tier machinery | Verdict | Why |
|---|---|---|
| Container platform (ECS/EKS) | **Excluded** | Traffic is low and bursty; Lambda's scale-to-zero fits a public reference registry better than a running cluster. |
| Multi-service decomposition | **Excluded** | Shared transaction boundary + a single team; the coupling cost would exceed the isolation benefit. |
| Event bus / async workers | **Excluded** | The only long-running job is CSV import, which fits inside a request with per-row reporting. Revisit if import size grows past the Lambda timeout. |
| CQRS / read models | **Excluded** | Read volume is served by indexes and a narrow geo projection. |
| RDS Proxy | **Deferred** | Constrained pool is adequate at current concurrency. Trigger to adopt: sustained concurrent executions approaching the instance's connection ceiling (see OQ-INFRA-4). |
| Multi-AZ / read replica | **Deferred** | Availability target (§13, QA-4) is business-hours best-effort, not an SLA. |

**What LITE does *not* relax:** the PII/consent boundary (§8), input validation, secrets handling, and least-privilege networking. Tier sizing governs *scaling and topology* machinery, never security controls.

### 12.5 ADR Index

| ID | Decision | Status | Consequence |
|---|---|---|---|
| **ADR-001** | Lite tier: modular monolith on a single Lambda; no containers, no service decomposition, no event bus. | Accepted | Cheapest topology that meets §13. Revisit when import size exceeds the Lambda timeout or concurrency nears the DB connection ceiling. |
| **ADR-002** | Next.js **static export** to S3/CloudFront; all server logic in NestJS. | Accepted | Forbids SSR/ISR/route handlers permanently. Buys trivial hosting, CDN caching, and no frontend runtime to operate; costs client-side-only data fetching and no server-rendered SEO for dynamic pages. |
| **ADR-003** | PII enforced by an **allowlist serializer** plus consent pinned in the Prisma `WHERE`, not by route guards alone. | Superseded in part by ADR-013 | Defense in depth: a new endpoint cannot leak PII by forgetting a guard. Costs one indirection on every read path; proven end-to-end in `src/test/pii-boundary.spec.ts`. |
| **ADR-004** | Consent-gated publication — public reads return **`GRANTED` actors only**, and exact GPS only for `GRANTED`. | Accepted | The registry under-reports its own dataset publicly by design. Non-negotiable: it is the legal/ethical basis for publishing at all. Widened by ADR-013: `GRANTED` no longer gates exact GPS alone — it now unlocks the actor's full disclosed field set on the detail read, GPS included. |
| **ADR-005** | **Prisma** as the sole DB access path (no raw SQL on user input). | Accepted | Parameterization by default; migrations are versioned artifacts. Costs Prisma's Lambda bundle size and cold-start weight. |
| **ADR-006** | **Cognito** for identity with group-based RBAC (`admin`, `staff`, else `Public`). | Accepted | No password storage or session infrastructure to own. Couples user administration to Cognito APIs (see `UsersModule`). |
| **ADR-007** | **Leaflet** (not a proprietary map SDK), loaded via a dynamically imported client component. | Accepted | No vendor key or per-view billing; keeps the map out of the static-export server pass. |
| **ADR-008** | **AWS SAM** as the only IaC; three ordered stacks. | Accepted | One provisioning tool matching the serverless target. Console-created resources are drift by definition. |
| **ADR-009** | Docs renamed to the AKILI baseline (`docs/ux-ui/design.md`, `docs/trd/trd.md`); existing TRD section numbers **not** renumbered. | Accepted | Archived specs and code comments keep valid `§n` citations; the cost is the structure map at the top of this file. |
| **ADR-010** | Public self-registration's write path is secured by **structural containment**, not the `Actor` allowlist/consent filters: a dedicated `Registration` table whose public surface returns at most `{reference}` or `{status, reviewNote}` and never the payload. Abuse is bounded by a per-container `@nestjs/throttler` plus persistent counters (`EmailSendBudget`, `RegistrationLookupAttempt`) for the controls a per-container limiter cannot make durable across cold starts. | Accepted | The registry's first unauthenticated write path. `PII_ALLOWLIST`/`NEVER_PUBLIC_FIELDS` cannot protect it, so confinement is structural and independently gated (`src/test/pii-boundary.spec.ts` extension, release gate — QA-12). Four tables now hold personal data with no retention policy (accepted risk, PRD OQ-4). Publication of an approved submission as a public `Actor` is a separate, later, admin-gated act (`docs/specs/admin/registration-review-queue/`) — this ADR covers intake only. |
| **ADR-011** | Client-side analytics is **consent-gated by non-injection** and **contained by layout placement**. GA4's `gtag.js` is not injected at all until the visitor accepts (rather than loaded with consent-mode `denied`), and it is mounted in `app/(public)/layout.tsx` so `(admin)` routes structurally cannot reach it. | Accepted | Zero custom events is the PII mitigation, not an omission: no call site exists that could carry an actor id or a directory search string to Google — ADR-010's containment-over-filtering argument applied to the client. Consequences: GA4's defaults include **city-level** geography derived from IP, which the `/cookies` notice must state (understating it was a real defect, caught in review; the disclosure moved from `/privacy` to `/cookies` under `docs/specs/legal/legal-notices-and-consent-copy/`, T-4/T-10 — see ADR-014); withdrawal is asymmetric — accepting takes effect immediately, rejecting only from the next page load, because `next/script` cannot unload an executed script, and this is disclosed rather than fixed. Verified live in production, not only by unit tests: `dataLayer` must receive the `arguments` object, and a plain `Array` is silently ignored by `gtag.js` while every `dataLayer`-shaped assertion still passes. |
| **ADR-012** | **Approve-to-publish is a single-transaction, compare-and-set, human-acknowledged act, and the created `Actor` is the only public artifact it produces.** The registry's one path from private submission to public record. Publication is per-record and never bulk, because each publication is a consent decision (D-3); the payload is published as submitted and never reviewer-edited, because an edited record would be published as consented-to when it is not what was consented to (D-4). The `Registration` row itself never becomes public, before or after adjudication — extending ADR-010's structural containment across the adjudication boundary rather than relaxing it at it. | **Accepted** | The act is irreversible within this spec's scope; there is no un-publish, and the quality of the human judgement it depends on is ungateable (DC-33). Consent provenance is carried verbatim from the stored acceptance record, satisfying ADR-004's publishable state through chunk 1's shared invariant rather than around it. |
| **ADR-013** | **Supersedes ADR-003 in part — consent is now the disclosure boundary, not field identity; ADR-003's allowlist-serializer tactic is retained and now also carries a structural list/detail asymmetry.** `consentStatus = GRANTED` unlocks every actor-supplied field on `GET /api/v1/actors/:id` (`PUBLICLY_DISCLOSED_FIELDS` in `pii-consent.policy.ts`), not only exact GPS, which was consent's sole field-level effect under ADR-004; `PII_ALLOWLIST` remains the one designated re-restriction point but is retained **empty**, with **zero runtime consumers** today. `toPublicListItem`/`toPublicDetail` (`role-aware.serializer.ts`) remain explicit literal picks — never spread-plus-delete, so a newly added schema column defaults to **not public** until an author deliberately discloses it, rather than leaking automatically the moment it is added — and `toPublicDetail` composes the list item **by name**, so the compiler fails if the two shapes drift. `CONTACT_BLOCK_FIELDS`, a strict subset of the disclosed set, is withheld by key and by value from `GET /api/v1/actors` — the response the map, dashboard, and CSV are all built from — which makes bulk contact harvesting **structurally** impossible rather than policy-forbidden; the CSV's own column allowlist is redundancy behind that boundary, not the boundary itself. | Accepted | Re-restricting a field later means adding it to `PII_ALLOWLIST` **and** removing it from `PUBLICLY_DISCLOSED_FIELDS`/`CONTACT_BLOCK_FIELDS` — the constant filters nothing by itself. A scripted caller can still page `GET /api/v1/actors/:id` per id: there is no rate limit on the public read paths, so the list/detail asymmetry raises the cost of bulk harvesting rather than eliminating it — an accepted cost, not a mitigated one. `NEVER_PUBLIC_FIELDS` stays absolute on every path regardless of consent; `technicalSupport` sits there for a different reason than its neighbours — unreviewed staff-authored text, never the actor's own declaration. Non-`GRANTED` actors are excluded in the Prisma `WHERE` (`ActorsService.findPublic`/`findOnePublic`), not only the serializer, and a non-`GRANTED` detail request `404`s indistinguishably from a missing id. **Amended 2026-09-15 — this open edge is closed** (`docs/specs/legal/legal-notices-and-consent-copy`, T-9). It previously read: *"The consent-policy text is still `[PLACEHOLDER TEXT — pending legal review, OQ-1]` and `CONSENT_POLICY_VERSION` was deliberately not bumped past `'v1.0-placeholder'` — existing actors were not re-consulted; a real open edge, not an oversight."* Legal delivered the approved copy and it ships as edition `v1.0`. The module is now an **append-only registry of editions** (ADR-014): `v1.0-placeholder` is retained forever rather than overwritten, so a `Registration.consentPolicyVersion` row pointing at it still resolves to the exact text that version meant, and a submission asserting it is still accepted. **The "existing actors were not re-consulted" half did not need closing:** public self-registration never reached production, so no registration was ever accepted under the placeholder version — recorded as decision D-8 of that spec, on the product owner's statement rather than on a query, since no environment holding real applicant traffic exists to query. One `Registration` row under `v1.0-placeholder` created by a real applicant would falsify D-8 and re-open the governance question. Revisit trigger: if `PUBLICLY_DISCLOSED_FIELDS` ever grows to include a materially more sensitive category than today's contact/location/demographic set (e.g. a financial or health-status field), the accepted-cost framing above was priced against today's field set and must be re-priced against the new one — that is the signal, not a request-volume threshold this system does not yet measure. |
| **ADR-014** | **Legal copy (Terms of Use, Privacy Policy, Cookie Notice) is version-controlled code, not database rows and not admin-editable.** `frontend/lib/content/legal/{terms,privacy,cookies}.ts` are the sole source; the consent-policy text in `backend/src/registrations/consent-policy.ts` already established the same pattern (ADR-013 area) and this ADR extends it to the frontend-rendered documents. | Accepted | Legal text is a release artifact, not user data — it changes ~annually and each change is a signed legal event, so git's who/when/diff trail substitutes for an audit table a DB row would need to grow. A legal text change requires a deploy — a static export plus, where the consent policy is involved, a Lambda update — not a migration; acceptable at this change rate. Non-engineers cannot edit legal copy directly; every change goes through a pull request, which is the intended property, not a limitation. An admin-editable CRUD (the rejected third option) is rejected **permanently, not deferred**: a `staff` user able to alter text a visitor has already legally accepted is a liability no UI convenience offsets. `Registration.consentPolicyVersion`'s target must stay retrievable and immutable forever, which an append-only code registry gives for free and a database table would need an append-only constraint plus UPDATE-locking plus per-environment seed choreography to match (`docs/specs/legal/legal-notices-and-consent-copy/`, D-1). |
| **ADR-015** | **Email leaves the system through the OneCGIAR Notification Microservice, reached over a RabbitMQ broker under a publisher confirm (`MicroserviceMailTransport`), not the now-removed Amazon SES** (`enhancement/email-notification-microservice`, `design.md` §1, DD-1…DD-11). SES is removed everywhere — code, IaC, developer IAM policy (FR-6) — because its sandbox required per-recipient verification, which blocked exactly the flow this spec exists to unblock: delivering an OTP to an applicant nobody has pre-verified. **Alternatives considered and rejected:** (a) keep SES and widen its sandbox — rejected, verification-per-recipient does not scale to an open public registry; (b) `@nestjs/microservices`' `ClientProxy` — rejected (DD-2), its background reconnection depends on timers that do not fire while a Lambda container is frozen, and its `emit()` resolves on local buffer write rather than broker acceptance, which would make the `202`/`502` distinction unfalsifiable. The transport writes no topology of its own (`checkQueue`, never `assertQueue` — DD-3), retries the **connection** at most once before a publish and never republishes after one (DD-4), and closes all three paths through which an `amqplib` error can leak the credential-bearing broker URL (DD-5). | Accepted | **The delivery guarantee is weaker than SES's, by construction, and this is the cost of the decision, not a side detail.** A confirmed publish means the broker persisted the message on its side — never that SMTP delivered it downstream. `202` now asserts only "durably enqueued"; the new `502` means **the broker did not confirm the publish within the send deadline** — *could not enqueue* — and must never be read as *could not deliver* (DD-3). **There is no retry past the publish and no dead-letter queue (D-H).** A message the microservice fails to render or fails to relay to SMTP is simply lost, and nothing in this system observes that loss — no callback, no bounce handler, no DLQ consumer. **The microservice posts every message's subject line to Slack (`#microservices-notifications`), not only failing ones** (`design.md` §6; `proposal.md` R-4 — *"to be stated in the ADR, not silently accepted"*): this is a full-send-volume disclosure surface, not a failure-only one, and when a send does fail, that same Slack post is the **only** signal that exists anywhere for D-H — and this system does not consume it. Every subject it can ever post there is scoped by construction: the approval, rejection, and receipt subjects carry only the applicant's public `reference`; the verification-code subject is a fixed string carrying neither the code nor any identifier (no `Registration` exists yet to reference — `verification-code.template.ts`'s own docblock says so); and the contact form's subject is a single fixed server constant, `CONTACT_SUBJECT`, never the visitor's own subject line — so, of the five kinds this system sends, none carries a recipient address. The channel's membership and retention are the platform team's, not this project's, and are not audited by this system. **The values `MailMicroserviceSecret` supplies resolve into plaintext Lambda environment variables** (`RABBITMQ_URL`, `MICROSERVICE_API_KEY`, `EMAIL_QUEUE_NAME` — DD-6, `design.md` §4.5, §7.2), readable by anyone who can call `lambda:GetFunctionConfiguration` on this function — the same exposure `DB_PASSWORD` and `OTP_HMAC_SECRET` already carried, extended rather than newly introduced by this decision. **The OTP endpoint's constant-time floor (NFR-7, DD-10) was re-derived from measurement, not reasoned in the abstract, and the first reasoned value was wrong.** `MAIL_SEND_TIMEOUT_MS=1200` / floor `2000` ms were arithmetic over unmeasured terms; five live sends against the production broker (2026-09-16) measured `receipt` 1132 ms, `approval` 1170 ms, and `verification` 1172 ms confirmed, while `contact` (1227 ms) and `rejection` (≥1200 ms) timed out at the old 1200 ms bound **and still delivered** — the old bound sat inside the observed distribution, the one place a constant-time bound must never sit, and it would have reported failure for mail that had already arrived. The bound was re-derived from that measurement, composed into a new OTP floor, so every verification-code request costs **at least 4 s** (⚠️ *dated magnitude, not a live restatement: the floor's value as of 2026-09-17, when F4 last revised its composition — recorded inline so it survives archiving (`CLAUDE.md`: archived specs are frozen records; a bare pointer loses its own referent the moment the archived copy stops being read); the live figure and exact composition are `design.md` §12.1. "At least", not "≈" — the floor is a floor, a minimum the code pads UP to, never an estimate that could land either side of it.*) — **a cost the product owner accepted explicitly, for now** (`design.md` OQ-11: 2026-09-17, shown the raise from a separate, earlier `3.8 s` statement on 2026-09-16 that remains on record unedited, and told the `4.0 s` figure rests on laptop measurements T-9 may revise — accepted provisionally, not as a settled number) rather than one this design minimizes away; the structural remedy (connecting during Lambda init instead of inside the request) is deferred pending T-9's Lambda-region latency numbers. |
| **ADR-016** | **Cognito's self-service password-reset mail is delivered by a `CustomEmailSender` Lambda trigger, not by Cognito** (`auth/forgot-password-delivery`, `design.md` §3–§6). Cognito keeps the whole reset state machine — it generates the code, expires it, verifies it, and enforces the password policy on the new one (FR-3); **only delivery moved**. The function decrypts the code against a customer-managed symmetric KMS key and publishes it over AMQP to the same OneCGIAR Notification Microservice channel as every other mail in this system (ADR-015), connecting per invocation and caching nothing (DD-3a) so nothing survives a frozen container. **Alternatives rejected:** (a) bring SES out of sandbox — the standing decision is that this system sends only through the microservice; (b) replace Cognito's reset flow with our own — rejected, it would have changed the security shape of a reset for a delivery problem. | Accepted | ⚠️ **The trigger is all-or-nothing per pool**: setting it routes *every* email this pool emits through this function, not only the reset. Unhandled sources raise loudly and visibly rather than failing silently (FR-2), which is deliberate — a silent no-op is how a missing IAM grant survived months in ATP-71. **It adds a new deployable unit inside `10-data-auth`**, a stack that ships only under `DEPLOY_INFRA=true`, so changing the function is slow by construction. ⚠️ **There is no rollback that restores the prior behaviour**: with SES excluded permanently, removing the trigger resumes Cognito's own sending on `COGNITO_DEFAULT`, not on the SES configuration the pool carried before. Rollback means degrading, never returning. **Three defect classes here have no automated gate** — the KMS grant chain, decryption against the real key, and delivery — because every suite in this repository mocks the AWS clients; a real reset against DEV is the only thing that closes them, and it did (2026-09-25: mail received, code worked, sign-in succeeded). |

## 13. Quality Attribute Scenarios (Non-Functional Requirements)

*(AKILI canonical section 3. Six-part scenarios: source · stimulus · artifact · environment · response · measurable response measure. Security, performance, scalability, and availability are always evaluated.)*

| ID | Attribute | Scenario | Response measure |
|---|---|---|---|
| **QA-1** | **Security (PII)** | An **anonymous visitor** issues a request to **any public read path** (`/actors`, `/actors/:id`, `/metrics`) against the **deployed API under normal operation**; the system serializes the response through the role-aware allowlist. | **Zero** occurrences of `NEVER_PUBLIC_FIELDS` (`technicalSupport`, `traderId`, `gpsAltitude`, `gpsAccuracy`, `registrationSource`, `consentMethod`, `consentObtainedAt`, `consentReference`) on **every** public path, list or detail alike. `CONTACT_BLOCK_FIELDS` (`phone`, `email`, `position`, `marketLocation`, `contactPerson`) are additionally **zero**-occurrence on every path **except** the single-actor detail read for a `GRANTED` actor, where FR-1 (`actors/public-profile-disclosure`) requires them present by value; a non-`GRANTED` actor's detail request `404`s and carries none of that actor's own fields. Asserted end-to-end over HTTP, by key and by value. Tactic: allowlist serializer (ADR-003, retained in part by ADR-013). Verified by `src/test/pii-boundary.spec.ts`. |
| **QA-2** | **Security (consent)** | An **anonymous visitor** requests actor data **including aggregate counts**, in the **deployed environment**, where the dataset contains `UNKNOWN`/`DENIED` actors; the query layer filters on consent. | **Zero** non-`GRANTED` actors in any public response **or in `/metrics` counts**; `gps: null` for every non-`GRANTED` record. Tactic: consent pinned in the Prisma `WHERE`, not the serializer (ADR-004). |
| **QA-3** | **Security (authorization)** | An **authenticated `staff` user** calls an **admin-only endpoint** (`DELETE /actors/:id`, `POST /import`, user management) in the **deployed environment**. | `403`, with a consistent error envelope and **no** stack trace or internal detail leaked. Tactic: `JwtAuthGuard` + `RolesGuard`, independent of the serializer. |
| **QA-4** | **Availability** | **Any user** loads the public map or directory during **Tanzanian business hours**, in **normal operation**. | Best-effort availability on managed AWS redundancy (CloudFront + Lambda + single-AZ RDS). **No formal SLA is claimed.** Frontend availability is decoupled from API health: a failed API call degrades to an error state, never a blank page. Multi-AZ deferred (§12.4). |
| **QA-5** | **Performance (interactive read)** | A **public visitor** applies a filter on the directory, with the **full ~1,000-actor dataset** loaded, under **normal operation**. | Paginated response returns fast enough to feel interactive at this dataset size, served by indexes on `region`, `traderType`, `consentStatus`, `traderName` — never a full scan or an unbounded list. **No numeric budget has been measured or agreed** (OQ-TRD-1). |
| **QA-6** | **Performance (map load)** | A **public visitor** opens the map view with the registry at or above the PRD's **1 000+** target. | The map plots **every** matching actor — it never paginates, clusters, or samples; a visitor cannot be shown "page 2 of Tanzania". It accumulates `GET /api/v1/actors` pages **in sequence**, so the **round-trip count**, not the payload, is the cost that scales: `DASH_PAGE_SIZE` **500** × `DASH_MAX_PAGES` **10** = a **5 000-actor ceiling**, with gzip above 1 KB (`common/compression.config.ts`). **Measured 2026-09-21** in a real browser over CDP — cold cache, 4 Mbps, +150 ms per request: **5 000 actors → 10 requests, 182 KB, complete map at 6.1 s**; 3 000 → 6 requests, 112 KB, 5.3 s. The rejected alternative is recorded because it is the obvious one: keeping 100/page and merely lifting the ceiling gave **50 requests, 1 404 KB, 15.1 s** at the same 5 000. Above the ceiling the shortfall is **stated on screen** ("Showing N of M actors"), never silently dropped — the prior build rendered 1 000 markers under the caption "1200 actors shown". ⚠️ **Measured on the local stack**, so it excludes Lambda cold start and the CloudFront/API-Gateway hop; treat the figures as a floor and re-measure against a deployed environment before quoting them as a deployed SLO. Verified by `frontend/components/map/DiscoverRail.test.tsx` (the shortfall statement, mutation-checked), `backend/src/actors/dto/actor-dto.spec.ts` (page-size boundary), `backend/src/common/compression.config.spec.ts` and `backend/src/test/lambda-handler.e2e.spec.ts` (gzip base64 through the real handler). |
| **QA-7** | **Performance (cold start)** | A **visitor** hits the API after an **idle period**, so the Lambda cold-starts. | Mitigated, not eliminated: lazy Prisma client **reused across invocations** and a minimal bundle. Accepted consequence of ADR-001's scale-to-zero choice. |
| **QA-8** | **Scalability (data + concurrency)** | The dataset **grows beyond the 1,000+ target** and concurrent Lambda executions rise, in the **deployed environment**. | List/map/query paths stay indexed and paginated. **Explicit trigger:** when sustained concurrency approaches the RDS instance's connection ceiling, adopt RDS Proxy (§12.4, OQ-INFRA-4). Static frontend scales on CloudFront with no change. |
| **QA-9** | **Correctness (bulk import)** | An **admin** uploads a **CSV containing invalid rows** in the **deployed environment**. | Valid rows commit; **a bad row never corrupts a committed row**. Response reports `{ inserted, updated, failed: [{ row, errors }] }` per row. Tactic: per-row validation + transactional upsert by `traderId`. |
| **QA-10** | **Observability** | An **operator** investigates a failed request **after the fact**. | Structured JSON logs in CloudWatch carrying request id, route, role, and latency; import jobs log summary counts. Client-facing errors carry a consistent envelope and **never** a stack trace. |
| **QA-11** | **Accessibility** | A **keyboard or screen-reader user** browses the public site. | WCAG 2.1 AA per `docs/ux-ui/design.md` §10; the **directory list is the accessible equivalent of the map**, so no information is map-only. Structural accessibility (labels, roles, keyboard reachability, ARIA) is enforced in frontend tests via `jest-axe`. **Colour contrast is not** — `jest-axe`'s `color-contrast` rule needs a layout/paint engine jsdom does not provide, so it is skipped without failing rather than enforcing anything. Contrast is instead enforced by a deterministic computed-ratio assertion over the token palette (`frontend/lib/contrast.test.ts`, `docs/specs/enhancement/app-visual-refresh`). |
| **QA-12** | **Security (unapproved PII)** | An **anonymous visitor** issues a request to **any of the four public self-registration paths** (`consent-policy`, `verify`, `registrations`, `registrations/lookup`), against the **deployed API**, where a `Registration` exists in every reachable status (`PENDING_REVIEW`, `APPROVED`, `REJECTED`). | **Zero** occurrences of any `payload` field value, `submitterEmail`, the internal `id`, or reviewer identity in any response body — asserted against fixture **values**, not key names, over HTTP, on every public path this module adds, for a Registration in every reachable status. Tactic: structural containment, not filtered access (ADR-010). Verified by the extended `src/test/pii-boundary.spec.ts`. **Release gate.** |
| **QA-13** | **Security (unapproved PII)** | An **anonymous visitor** issues a request to `POST /api/v1/contact`, against the **deployed API**, where the request carries a requester name, organization, category, subject and message, and the mail transport rejects the send. | **Zero** occurrences of any requester field value or any administrator email address in any response body, error envelope, redirect target, **or log line** produced by `POST /api/v1/contact` — asserted against fixture **values**, not key names, end-to-end over HTTP. Tactic: stateless relay with no persistence (DD-1), server-side recipient resolution never returned to the caller (§6). Verified by the extended `src/test/pii-boundary.spec.ts`, with a `Logger.prototype` spy over `log`/`warn`/`error` — `MailService` is provider-overridden in that block, so the line actually gated is `ContactService`'s transport-failure catch, which is where a transport rejection can carry a recipient address verbatim in its message — true of AWS SES's now-removed `MessageRejected` and equally true of the message-broker transport that replaced it (`enhancement/email-notification-microservice`, ADR-015), so there is no transport-specific list of "safe" error shapes to maintain. Run against a deliberately-leaking variant (`err.name` → `err.message`) to confirm the gate fails when it should, and guarded against vacuity by asserting the spy captured at least one call before sweeping it. **Release gate.** |

**B3 resolution (`docs/specs/admin/registration-review-queue/judgment.md`).** The five `admin/registrations/*` endpoints that spec adds are covered by **QA-3**, not by a widened QA-12. QA-12's actor is an anonymous visitor on a closed set of four *public* self-registration paths — a structural-containment scenario; `admin/registration-review-queue` adds no public path, and its five routes are Admin-gated with exactly QA-3's `401`/`403` shape. Widening QA-12's actor text to cover admin-gated PII routes would blur two genuinely different scenarios — public-surface containment vs. admin-endpoint authorization — under one label, so QA-3 is cited alone and QA-12's text is unchanged. **The two scenarios do share a mechanism, which is the actual source of the ambiguity B3 raised:** ADR-010 ties QA-12 to the `pii-boundary.spec.ts` extension, and `admin/registration-review-queue` extends that *same* file (via a `FIXTURE_MAP` access discriminant) to prove QA-3 for its five routes. One test file now serves two distinct QA scenarios — recorded here so a future reader who notices that overlap does not mistake it for a reason to re-open B3.

**Open question OQ-TRD-1:** QA-5 has no agreed numeric latency budget. "Fast enough to feel interactive" is not a testable response measure — a p95 target (e.g. filtered list p95 < 500 ms at 1,000 actors) should be set and measured before it can be asserted in a test. Recorded honestly rather than invented here.
