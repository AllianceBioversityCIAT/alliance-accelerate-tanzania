...
**Two rows were removed here on 2026-09-21 — deliberately, and they must not come back (ATP-53, ATP-68).**
...
**QA-1 (§13) was trimmed to match.** Its public-read-path list named `/export` and `/actors/geo`; both are now absent from it because neither route exists. If the map decision (ATP-68, still open) creates `/actors/geo`, it must be re-entered in QA-1 **and** QA-6 — a quality scenario naming an absent route verifies nothing.
